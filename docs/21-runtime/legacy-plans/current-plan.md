...
### ADR-9: 认知引擎落盘三档口径（PMO-59 Phase 2，2026-09-14 正式落盘）

**决策**: cognitive-engine（木·C 认知）的落盘边界从"一律不落盘"修订为三档口径——铁律 §0.3 表 cognitive 行"禁止新增 DB 表（推理实时计算）"与 §3.3 对应条文同步修订为本口径，其余条款零改动。

| 档位 | 口径 | 落地 |
|:--|------|:--|
| ① 推理"结果" | **不落盘**，实时计算（原口径保留） | 因果链/诊断/推演结果仅响应体返回； KG 查询/SpEL 评估不产生持久化表 |
| ② 模型"资产" | **落盘**（ADR-8 修订口径，PMO-51 确立） | `ecos_cognitive_model`（DDL V124：模型注册/版本/回测分） |
| ③ 认知"心智状态" | **落盘**（本 ADR 解锁，PMO-59） | 三表（DDL V127~V129，手工执行）：`ecos_cognitive_evidence`（证据/来源/可信度/冲突标记）、`ecos_cognitive_hypothesis`（假设/证据引用/valid/失效时间）、`ecos_cognitive_belief`（不确定性判断：变量名/有限离散概率分布 JSONB/版本/`snapshot_version` 时间回放预留/人工覆写标记） |

**理由**:
- 心智状态（证据/假设/不确定性判断）是**跨请求持续状态的实体**，与瞬时推理结果本质不同：失效检测需要历史证据链路（同 fact+metric 多值冲突比对），不确定性判断需要版本递进 + 证据溯源（`last_evidence_id`），时间回放需要历史版本引用（`snapshot_version`）——不落盘则心智层能力（外部方案层 3.1/3.2）无法实现。
- 三表均带完整审计六列 + `is_deleted` 逻辑删除 + `uniq_*`/`idx_*` 索引，与铁律 §3.1 数据层规范一致。
- 术语口径：业务方统一称"不确定性判断"（外部方案原稿"信念"已改称，避心理学歧义）。

**代价/约束**:
- cognitive-engine 后续**仍不允许**新增心智三表与模型注册表之外的 DB 表（Phase 3 时间回放若新增表，须先走新 ADR 并同步修订本条）。
- 分布 prob 和=1 无 DB CHECK（JSONB 动态结构），由 Service 层强校验（容差 1e-6）。
- LLM 零参与认知计算（ADR-5 口径），加权贝叶斯更新为纯 Java 规则计算。

**同步修订**: `.trae/rules/架构铁律.md` §0.3 表 cognitive 行禁止列 + §3.3 跨引擎数据访问（P2a T5 执行，仅改"cognitive 不新增 DB 表"表述，其余条款零改动）。
