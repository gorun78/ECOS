...
>   > **PMO-59 P4a 告警落库通道（2026-09-16，runtime-monitor 补强而非自建；零新增 REST 端点）**：`IWarnLogService` 新增七参 `warn(type,objid,objname,err,typeHander,faultContext,reviewTag)` default 重载（默认委托既有五参，签名只增不改），`WarnLogServiceImpl` 保留内存 logStore（查询语义零变化）+ **只增持久化分支**落 `ecos_warn_log`（DDL V131：id/log_id(uniq 幂等)/warn_type/warn_level/warn_objid/warn_objname/warn_message/fault_context JSONB/review_tag/warn_hand/warn_result/ishanded/warn_time + 审计六列 + is_deleted；4 普通索引）；`JdbcTemplate` 构造器注入（null=纯内存态向后兼容），落库异常 try/catch WARN 不抛出（不阻塞主流程）。
>   > 复盘口径对接：`GET /api/v1/cognitive/mental-reviews` 输出**只增键** `warnAlerts[]`（按 `review_tag` = 复盘 tag 同源等值 + since 只读查询）+ `summary.warnAlerts` 计数；既有 5 段结构与键 0 变化；V131 未执行时该段降级空列表。
>   > 告警查询口径（勘察裁定，本期不新增端点）：既有 `GET /api/monitor/alerts`（统计，源 `ecos_alert_history`）与 `GET /api/v1/alerts`（列表，:8080 由 workspace 降级路由承接）不变；`ecos_warn_log` 本期以 psql 直查 + 上述 `warnAlerts` 段为查询通道。
