...
| 批次 | 主题 | 覆盖 |
|:--|:--|:--|
| B1 | 图谱骨架修复（读 `kb_ontology_snapshot` 替代幻影表 + 异常不吞） | D1、D9 |
| B2 | 事件 payload 补全 + 映射校验端点 | D7、D8 |
| B3 | DW 实例读取端点 + 图谱实例抽取（契约驱动，含 `materialized` 列） | D2、Q2 |
| B4 | 向量链路修复（新增 vector 列 + HNSW + chunk 写入） | D3、D4 |
| B5 | 非结构化登记链路 + 6 个 404 端点补齐（含 `kb_doc_chunk` 过渡表） | D5、D6、Q1 |
| B6 | 管道 `SOURCE_MINIO` + 解析节点（A1 目标态） | D5、D10 |
| B7 | 越界修正 + 规范文档同步（Q6 需单独授权改规则文件） | D11、D12、Q6 |
| B8 | Tab 分组重排（7 组）+ `knowledge.group.{store,govern}` i18n 词条补齐，**排在 B5 之后** | Q5 |
