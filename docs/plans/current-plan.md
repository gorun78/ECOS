...
> **日期**: 2026-09-10
> **版本**: v1.2（架构评审修订）
> **变更记录**: v1.1 → v1.2：统一拆分口径为"5 业务服务 + 1 网关"；gateway 模块定位为顶层目录；新增 ADR-7（服务间信任与认证）、ADR-8（审计通道统一 Kafka）；破除铁律清单补齐（Flyway/Schema 重组/审计通道/三滤波器重分布/端口互斥）；铁律修订前置至 Phase 1；新增 dccheng 双引擎合并 PoC；三档位术语统一为 ultimate；修复 mermaid/gantt/验收端口错误。详见 §9。
...
将 ECOS 后端从**单一 fat-JAR（gateway）** 拆分为 **6 个独立可部署的 Spring Boot 微服务** + 1 个 API 网关。六引擎（security/data/ontology/kb/cognitive/ai）作为独立可运行的服务层保持不变，各自已有 `*-engine-boot` 模块可独立启动。

核心理念：**六引擎是"发动机"（独立能力），服务层是"底盘+车身"（业务封装 + 编排），网关是"方向盘"（路由入口）**。

- **多任务并行**: 各服务模块独立编译、测试、部署，互不阻塞
- **独立扩缩容**: 按负载特征独立伸缩
- **故障隔离**: 单服务故障不拖垮整个系统
- **三档位保持**: standard/enterprise/flagship 三套发布模式通过服务开关控制

### 1.2 改造前后对比

| 维度 | 改造前（单体） | 改造后（微服务） |
|------|---------------|-----------------|
| 部署单元 | 1 个 fat-JAR（gateway:8080） | 5 个业务 service + 1 个网关（共 6 个部署单元）+ 基础设施容器 |
...
| c) 本体服务 | `services/buszhi` | ontology-engine + zhi 转化 | 18083 | `ontology-engine-impl` | 本体建模/对象/关系/版本 + 工作流引擎 |
...
| — | `services/gateway` | —（纯网关） | 8080 | `common-api` + security-engine-api（认证验证） | 统一入口/路由/认证前置/限流/监控聚合 |

**引擎独立性**:
- 六引擎各自的 `api`/`impl`/`boot` 三模块结构**完全不改动**
- 各引擎的 `boot` 模块保留，用于引擎独立启动调试
...
/services/buszhi/pom.xml ──(depends on)──▶ engine/ontology-engine-impl ──▶ engine/ontology-engine-api ──▶ common-api
...
**方案 A（采纳）**: 同步调用用 REST（`RestTemplate`），异步事件用 Kafka。

- 同步调用：各 service 通过 `RestTemplate` 调用其他 service 的 `/api/v1/...` 端点
- 异步事件：通过 Kafka topic 传递 `PipelineEvent`
...
- **enterprise/flagship 档**: 每 service 独立 PG database
...
- `ecos-shared-lib`（新建）: 共享 Entity/DTO（`BaseEntity`、`Tenant`、`User` 等跨 service 引用的实体）
...
- Topic: `ecos.audit.events`（消息格式见 [api-contract.md](api-contract.md) §5）
...
graph TB
    ...
    GW -->|/api/v1/ecos/*| BZ
    GW -->|/api/v1/kb/* /api/v1/rules/*<br/>/api/v1/cognitive/* /api/v1/world-model/*<br/>/api/v1/knowledge/reason| DCC
...
| 隔离级别 | 每 service 独立 Spring Boot JAR | 5 个业务 service + 1 个 gateway = 6 个部署单元 |
| 最大服务数 | 8-12 个 | 5 service + 1 gateway = 6 个部署单元（符合范围） |
| 数据隔离 | 每 service ≥ 1 个独立 PG schema | ADR-3 |
| 审计日志 | 必须 | 各 service 发 Kafka `ecos.audit.events`，sysman 消费落库（ADR-8） |
...
gantt
    ...
    清理 gateway 依赖     :p2f, after p2e, 3d
...
| P1-2 | 仓库结构重组 | `services/{sysman,datanet,buszhi,dccheng,aiming}` 目录创建；每目录含 `pom.xml` + `docker/Dockerfile` 占位 |
...
| P1-5 | 本地构建验证 | 所有模块 `mvn install` 成功 |
...
| P1-7 | **架构铁律修订前置**（原 P7-5 提前） | 按 §8 清单完成铁律文档修订（§0.1 单体→微服务、§0.3 boot 角色、§2.4-5 审计通道、§1.2 三滤波器重分布、端口互斥、5.1#10/#4 基线更新）——改造全程引用的"宪法"必须先与目标架构对齐 |

**里程碑**: 仓库编译通过，服务目录结构就绪，dccheng 合并风险已验证，铁律与目标架构对齐
...
| P2-3 | **buszhi service**: 创建 `BuszhiServiceApplication`，扫描 `com.chinacreator.gzcm.engine.ontology` + `com.chinacreator.gzcm.buszhi`；本体 CRUD + 工作流通过 | `curl GET :18083/api/v1/ecos/object/list` → 200 |
| P2-4 | **dccheng service**: 创建 `DcchengServiceApplication`，扫描 `com.chinacreator.gzcm.engine.kb` + `com.chinacreator.gzcm.engine.cognitive`；KG/规则/推理全通过 | `curl POST :18086/api/v1/kb/graph/sync` → 200; `curl POST :18086/api/v1/cognitive/diagnose` → 因果链 |
...
| P2-6 | **gateway 僵尸清理**: 删除 `GatewayApplication` 中已迁出引擎的 `@ComponentScan` 包 + 对应 `excludeFilters`；保留 gateway 自有 Controller | `mvn install` 全量成功 |
...
- 审计事件生产者接入 `ecos.audit.events`（ADR-8）

**里程碑**: 5 个 service 独立启动通过，gateway 保留（双跑验证）
...
| 任务 | 内容 | 验收标准 |
|------|------|---------|
| P7-1 | **废弃目录清理** | 删除已无引用的旧目录：原 `ecos_backend/sysman/`（已合并至 `services/sysman/`）、原 `ecos_backend/buszhi/`（已合并至 `services/buszhi/`）、原 `ecos_backend/common/`（已迁入 `runtime/common-api`）、原 `ecos_backend/shared-lib/`（已迁入 `runtime/ecos-shared-lib/`）、已吸收删除的旧 JAR 目录残留。**引擎 `boot` 模块保留**（仅限独立调试，非生产入口），在对应 AGENTS.md 标注 |
| P7-2 | **无用 POM 清理** | 检查根 `pom.xml` 的 `<modules>` 列表，移除已不存在的模块引用；彻底删除三级空目录（保留 git 占位） |
| P7-3 | **Docker 更新** | 更新 `ecos-docker/docker-compose.yml`：删除旧的 10 个废除微服务定义（`ecos/*-service` 无构建来源的）；新增 5 个 service 聚合 + 1 个 gateway 的 compose 定义 |
...
| R9: sysman 同步扇入热点 | 高 | 高 | 安全裁决本地缓存 + `ecos.security.events` 失效广播（ADR-2）；缓存命中率纳入监控 |
...
| 2.4-5 | 审计异步调 `POST /api/v1/security/audit/log` | 改为发 Kafka `ecos.audit.events`，sysman 消费落库（ADR-8）；REST 审计端点过渡期保留 |
...
│   │   └── docker/Dockerfile
│   ├── buszhi/                               (新建: 封装 ontology-engine + 原 buszhi/ 合并)
│   │   ├── pom.xml
...
│       └── docker/Dockerfile
└── gateway/                                  (原顶层 gateway 迁入 services 后，回归顶层)
    ├── pom.xml
...
> 4. 原顶层 `buszhi/` → 合并到 `services/buszhi/`
> 5. `services/gateway` → 提取回顶层 `gateway/`（与 engine 平级，作为独立可部署网关）
