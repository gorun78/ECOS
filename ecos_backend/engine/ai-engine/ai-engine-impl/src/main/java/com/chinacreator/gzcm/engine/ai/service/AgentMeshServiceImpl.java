...
public class AgentMeshServiceImpl implements AgentMeshService {
    ...
    @Override
    public Map<String, Object> routeIntentByLLM(Map<String, Object> req) {
        String query = (String) req.getOrDefault("query", "");
        List<AgentRegistryEntity> agents = agentRepo.findActive();

        String bestAgentId = "default";
        String bestAgentName = "Default Agent";
        double bestScore = 0.0;
        String bestReasoning = "No matching agent found";

        for (AgentRegistryEntity agent : agents) {
            double score = computeSemanticScore(query, agent);
            if (score > bestScore) {
                bestScore = score;
                bestAgentId = agent.getId();
                bestAgentName = agent.getName();
                bestReasoning = "Matched agent " + agent.getName() + " with score " + score;
            }
        }

        Map<String, Object> result = new LinkedHashMap<>();
        result.put("query", query);
        result.put("targetAgentId", bestAgentId);
        result.put("targetAgentName", bestAgentName);
        result.put("confidence", bestScore);
        result.put("routingStrategy", "llm_semantic");
        result.put("reasoning", bestReasoning);
        return result;
    }

    private double computeSemanticScore(String query, AgentRegistryEntity agent) {
        double score = 0.0;
        String q = query.toLowerCase();

        if (agent.getRole() != null) {
            for (String word : agent.getRole().toLowerCase().split("[,\\s]+")) {
                if (q.contains(word) && word.length() > 1) score += 0.3;
            }
        }

        String metadata = agent.getMetadata() != null ? agent.getMetadata().toLowerCase() : "";
        for (String word : q.split("[,\\s?！？，。]+")) {
            if (word.length() > 1 && metadata.contains(word)) score += 0.2;
        }

        String capability = agent.getCapability() != null ? agent.getCapability().toLowerCase() : "";
        for (String word : q.split("[,\\s?！？，。]+")) {
            if (word.length() > 1 && capability.contains(word)) score += 0.15;
        }
