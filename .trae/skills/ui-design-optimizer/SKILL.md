...
version: 1.4.0
description: 优化前端实现以匹配 UI 设计规格，基于证据的验证。当需要将前端代码与设计文件对齐、分析设计规格或验证设计还原准确性时调用此技能。
category:
...
通过系统化分析、优先级优化和证据驱动验证，将前端实现与 UI 设计规格对齐。本技能解决设计与实现之间常见的视觉不一致问题，确保高保真还原。

## 核心原则

基于真实项目经验，这些原则确保有效的设计优化：

### 原则 1：证据驱动分析（40% 失败原因）
- 为每个设计规格收集可验证的证据
- 记录所有颜色/值引用的文件路径和行号
- 生成带有可追溯来源的结构化分析报告
- **关键**：同时分析主题变量和组件实际使用，检测颜色冲突
...
| 组件 | 组件中使用的颜色 | 主题变量 | 匹配 | 解决方案 |
|------|-----------------|---------|------|----------|
| 按钮 | #3370FF | --primary: #030213 | ❌ 不匹配 | 使用组件颜色 (#3370FF) |
| 标签页激活 | #3370FF | --primary: #030213 | ❌ 不匹配 | 使用组件颜色 (#3370FF) |
| 边框 | rgba(51,112,255,0.1) | --border: rgba(0,0,0,0.1) | ❌ 不匹配 | 使用组件颜色 |

### 配色方案对比

| 优先级 | 调整项 | 设计稿规格 | 当前值 | 差异 | 设计稿来源 | 代码来源 | 代码片段 |
|--------|--------|-----------|--------|------|-----------|---------|----------|
| P0 | 主色 | #3370FF | #030213 | 颜色不匹配 | index.tsx#L100 | style.css#L8 | `--primary: #030213` |
| P0 | 主色浅色 | #4E83FD | #030213 | 颜色不匹配 | index.tsx#L105 | style.css#L9 | `--primary-light: #030213` |
| P0 | 背景色 | #ffffff | #ffffff | ✅ 一致 | theme.css#L5 | style.css#L25 | `--bg-page: #ffffff` |

### 排版对比

| 优先级 | 调整项 | 设计稿规格 | 当前值 | 差异 | 设计稿来源 | 代码来源 | 代码片段 |
|--------|--------|-----------|--------|------|-----------|---------|----------|
| P1 | 正文字号 | 14px | 14px | ✅ 一致 | theme.css | style.css | `font-size: 14px` |
```
...
| 列名 | 描述 | 格式 |
|------|------|------|
| 设计稿来源 | 设计稿文件路径和行号 | `filename.css#Lxx` 或 `filename.tsx#Lxx` |
| 代码来源 | 前端代码路径和行号 | `filename.css#Lxx` |
| 代码片段 | 关键代码片段 | `--variable: value` 或 `color: #xxx` |
...
|------|-----------|--------|------|----------|
| 主色 | #030213 | | ✅/❌ | `getComputedStyle(root).getPropertyValue('--primary')` |
...
**变更记录**：
- **v1.4.0** (2026-07-24): 添加设计稿颜色冲突检测（关键）
