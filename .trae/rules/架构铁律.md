...
| **cognitive-engine** | 木（C 认知） | 18084（PMO-60 后 run in aiming） | 因果推理/情景推演/混合推理 | 落盘三档口径（ADR-9）：推理**结果**不落盘(实时计算)；模型**资产**落 `ecos_cognitive_model`(ADR-8)；认知**心智状态**三表 `ecos_cognitive_evidence`/`ecos_cognitive_hypothesis`/`ecos_cognitive_belief`(V127~129) 落盘，不新增其他表；不引入规则引擎(SpEL即可) |
