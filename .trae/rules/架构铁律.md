...
> 版本: 1.1 | 2026-09-11 | 基于 Phase 1-2 实战教训 + 后端AGENTS.md + 前端GEMINI.md + 六引擎契约 + PMO-49 v1.4 微服务 v2
...
| **cognitive-engine** | 木（C 认知） | 18089 | 因果推理/情景推演/混合推理 | 落盘三档口径（ADR-9）：推理**结果**不落盘(实时计算)；模型**资产**落 `ecos_cognitive_model`(ADR-8)；认知**心智状态**三表 `ecos_cognitive_evidence`/`ecos_cognitive_hypothesis`/`ecos_cognitive_belief`(V127~129) 落盘，不新增其他表；不引入规则引擎(SpEL即可) |
...
| `services/aiming` | 智能 / 火 W | 18084 | `ai-engine-impl` + `llm-gateway` |
| `services/dccheng` | 知识库 / 水 K + 木 C | 18086 | `kb-engine-impl` + `cognitive-engine-impl` |
...
| 2 | `sysman/service/.../security/SecurityConfig.java` | permitAll加`/api/v1/XXX/**` 和 `/api/XXX/**` 两种形式（sysman service 内生效；其他 service 由各自 `HeaderAuthInterceptor` 委托 gateway 前置校验结果，见 ADR-7） |
| 3 | `sysman/service/.../security/ClearanceInterceptor.java` | 豁免列表加`/api/v1/XXX` 和 `/api/XXX` 两种形式（同上，检查点适配 sysman service） |
