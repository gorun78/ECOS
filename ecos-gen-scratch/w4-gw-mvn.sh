#!/usr/bin/env bash
# w4-gw-mvn.sh — use mvn spring-boot:run, NOT the runner jar
set +e
...
exec mvn -f /home/guorongxiao/ECOS/ecos_backend/pom.xml \
  spring-boot:run -pl gateway \
  -Dspring-boot.run.profiles=enterprise \
  -Dmaven.test.skip=true -Dnonexisting.skip=true \
