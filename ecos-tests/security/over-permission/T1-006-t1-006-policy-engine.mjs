...
try {
  if (!(await gatewayAlive())) {
    console.log("[ENV_BLOCKED] case: T1-006 单租户/Policy Engine 无 token");
    process.exit(77);
  }
  ...
  report("T1-006 单租户/Policy Engine 无 token (status=" + r.status + ")");
} catch (e) {
  reportFail("T1-006 单租户/Policy Engine 无 token", e);
}
