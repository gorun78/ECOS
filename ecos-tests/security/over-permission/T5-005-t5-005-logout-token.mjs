...
try {
  if (!(await gatewayAlive())) {
    console.log("[ENV_BLOCKED] case: T5-005 契约/Logout 无 token");
    process.exit(77);
  }
  ...
  if (bad) {
    throw new Error("默认 DENY 被突破: POST /api/v1/auth/logout 无 token 收到 " + r.status);
  }
  report("T5-005 契约/Logout 无 token (status=" + r.status + ")");
} catch (e) {
  reportFail("T5-005 契约/Logout 无 token", e);
}
