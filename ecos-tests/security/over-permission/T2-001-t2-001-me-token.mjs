...
try {
  if (!(await gatewayAlive())) {
    console.log("[ENV_BLOCKED] case: T2-001 单用户/Me 无 token");
    process.exit(77);
  }
  ...
  if (bad) {
    throw new Error("默认 DENY 被突破: GET /api/v1/auth/me 无 token 收到 " + r.status);
  }
  report("T2-001 单用户/Me 无 token (status=" + r.status + ")");
} catch (e) {
  reportFail("T2-001 单用户/Me 无 token", e);
}
