...
export default function CreateAgentModal({ form, onChange, onConfirm, onClose }: CreateAgentModalProps) {
  ...
  return (
    <div className={`fixed inset-0 z-50 flex items-center justify-center ${styles.overlayBg} backdrop-blur-sm`}>
      <div className={`rounded-xl shadow-2xl w-full max-w-md mx-4 p-6 border ${styles.cardBg}`} style={{ borderColor: 'var(--card-border, #E2E8F0)' }}>
        ...
        <div className="space-y-3">
          <div>
            <label className={`text-xs font-semibold ${styles.cardTextMuted} block mb-1`}>{t("agent.create.name")}</label>
            <input type="text" value={form.name}
              onChange={e => onChange(prev => ({...prev, name: e.target.value}))}
              className={`w-full border rounded-lg px-3 py-2 text-xs outline-hidden focus:border-indigo-400 ${styles.inputBg} ${styles.inputText} ${styles.inputBorder}`}
              placeholder={g(locale, "e.g. Data Analyst Agent", "如: 数据分析Agent")} />
          </div>
          <div>
            <label className={`text-xs font-semibold ${styles.cardTextMuted} block mb-1`}>{t("agent.create.role")}</label>
          ...
          <div>
            <label className={`text-xs font-semibold ${styles.cardTextMuted} block mb-1`}>{t("agent.create.goal")}</label>
            <textarea value={form.goal} rows={2}
              onChange={e => onChange(prev => ({...prev, goal: e.target.value}))}
              className={`w-full border rounded-lg px-3 py-2 text-xs outline-hidden focus:border-indigo-400 resize-none ${styles.inputBg} ${styles.inputText} ${styles.inputBorder}`}
              placeholder={g(locale, "What should this agent accomplish?", "该 Agent 的目标是什么？")} />
          </div>
          <div>
            <label className={`text-xs font-semibold ${styles.cardTextMuted} block mb-1`}>{t("agent.create.prompt")}</label>
            <textarea value={form.systemPrompt} rows={3}
              onChange={e => onChange(prev => ({...prev, systemPrompt: e.target.value}))}
              className={`w-full border rounded-lg px-3 py-2 text-xs outline-hidden focus:border-indigo-400 resize-none font-mono ${styles.inputBg} ${styles.inputText} ${styles.inputBorder}`}
