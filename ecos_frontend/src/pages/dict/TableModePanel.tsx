    ...
    <div className={`flex-1 ${styles.cardBg} overflow-y-auto`}>
      <div className="p-6 space-y-5">
        ...
        <div className={`grid grid-cols-2 gap-4 p-4 rounded-xl border ${styles.appBorder} ${styles.appBg}`}>
          <div>
            <div className={`text-[11px] font-semibold ${styles.cardTextMuted} mb-1`}>表名 *</div>
          ...
          <div>
            <div className={`text-[11px] font-semibold ${styles.cardTextMuted} mb-1`}>中文名称</div>
          ...
          <div>
            <div className={`text-[11px] font-semibold ${styles.cardTextMuted} mb-1`}>Schema / 库名</div>
            <input className={`w-full px-3 py-2 rounded-lg border ${styles.inputBorder} ${styles.inputBg} text-xs ${styles.cardText} outline-none focus:border-indigo-400 disabled:opacity-50 font-mono`}
              placeholder="例如：public" value={tableFormSchema} onChange={e => setTableFormSchema(e.target.value)} disabled={saving} />
          </div>
          <div>
            <div className={`text-[11px] font-semibold ${styles.cardTextMuted} mb-1`}>数据源类型</div>
          ...
          <div className="col-span-2">
            <div className={`text-[11px] font-semibold ${styles.cardTextMuted} mb-1`}>描述</div>
          ...
          <div className="col-span-2">
            <div className={`text-[11px] font-semibold ${styles.cardTextMuted} mb-1`}>标签（逗号分隔）</div>
            <input className={`w-full px-3 py-2 rounded-lg border ${styles.inputBorder} ${styles.inputBg} text-xs ${styles.cardText} outline-none focus:border-indigo-400 disabled:opacity-50`}
        ...
        {transitions.length > 0 && (
          <div className="flex items-center gap-2 flex-wrap">
            <span className={`text-[11px] font-semibold ${styles.cardTextMuted}`}>状态流转:</span>
            {transitions.map(tr => (
        ...
        <div className={`flex gap-2 pt-2 border-t ${styles.cardBorder}`}>
          ...
          <button className={`px-4 py-2 rounded-lg ${styles.appBg} hover:${styles.sidebarBg} ${styles.cardText} text-xs font-semibold transition disabled:opacity-50`}
