        ...
        <div className="flex-1 overflow-y-auto">
          {dictLoading ? (
            ...
            (Object.keys(subsystemGroups).length > 0 ? Object.keys(subsystemGroups).sort() : ["G1","G2","G3","G4","G5"]).map(groupKey => {
              ...
              return (
                <div key={groupKey} className={`border-b ${styles.cardBorder}`}>
                  ...
                  {isExpanded && visibleTypes.map(dt => {
                    ...
                    return (
                      <div key={dt.dictType}
                        className={`flex flex-col gap-1 pl-7 pr-3 py-2 border-b ${styles.cardBorder} cursor-pointer transition text-xs ${isActive ? "bg-emerald-50 border-l-2 border-l-emerald-500" : `hover:${styles.appBg}`}`}
                        onClick={() => handleSelectDictType(dt.dictType)}>
                        <div className="flex items-center gap-2">
                          <FolderTree size={12} className={isActive ? "text-emerald-500" : styles.cardTextMuted} />
                          <span className={`flex-1 font-semibold truncate ${isActive ? "text-emerald-700" : styles.cardText}`}>
                            {dt.dictName || dt.dictType}
                          </span>
                          {dt.itemCount != null && (
                            <span className={`text-[10px] ${styles.cardTextMuted} ${styles.appBg} px-1.5 py-0.5 rounded-full`}>{dt.itemCount}</span>
                          )}
                        </div>
                        <div className={`text-[10px] ${styles.cardTextMuted} pl-4`}>
