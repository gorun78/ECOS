...
export default function SecurityCenter({
 ...
 return (
 <div className={`h-full w-full flex ${styles.appBg} ${styles.appText} overflow-hidden font-sans select-none relative`}>
 ...
 activeTab === 'overview'
 ? `${styles.sidebarActiveBg} ${styles.cardText} border-l-2 ${styles.accentBorder}`
 ...
 <div className={`h-px ${styles.cardBorder} my-2`} />
 <button
 ...
 >
 <HelpCircle size={13} className={showCopilot ? 'text-blue-300' : styles.cardTextMuted} />
 ...
 {/* VIEW TITLE BAR */}
 <div className={`h-14 ${styles.cardBg} border-b ${styles.cardBorder} px-6 flex items-center justify-between shrink-0`}>
 <div className="flex items-center gap-3">
 <div className={`text-sm font-black ${styles.cardText} flex items-center gap-2`}>
 <span>Palantir Security Guardian</span>
 <span className={`text-[10px] ${styles.badgeBg} ${styles.cardTextMuted} px-2 py-0.5 rounded font-mono uppercase`}>
 Enterprise Shield
 </span>
 </div>
 </div>
 ...
 <TrendingUp size={13} className={styles.accentText} />
 ...
 <PieChartIcon size={13} className={styles.accentText} />
 ...
 <span className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`}>
 <Zap size={14} className={`${styles.accentText} animate-pulse`} />
 零信任策略仿真演算器 (Zero-Trust Security Policy Simulator)
 </span>
 <p className={`text-[10px] ${styles.cardTextMuted}`}>模拟不同用户、敏感资产和具体分析目的在 ECOS 级联策略链条中的真实决策判定结果</p>
 </div>
 <button
 ...
 {/* Inputs */}
 <div className={`space-y-3 ${styles.appBg}/80 p-3.5 rounded-lg border ${styles.cardBorder}`}>
 <div>
 ...
 className={`w-full ${styles.inputBg} border ${styles.inputBorder} rounded px-2.5 py-1.5 text-xs ${styles.inputText} font-mono outline-none focus:${styles.accentBorder}`}
 />
 ...
 {/* Verdict & Details */}
 <div className={`md:col-span-2 ${styles.appBg}/80 p-3.5 rounded-lg border ${styles.cardBorder} flex flex-col justify-between`}>
 {simResult ? (
 ...
 <div className="flex items-center justify-between">
 <div>
 <h3 className={`text-sm font-black ${styles.cardText}`}>ECOS 组织边界配置 (Organizations Setup)</h3>
 <p className={`text-[10px] ${styles.cardTextMuted}`}>ECOS 最高级别安全隔离机制。即使授予了文件权限，非共享协议白名单下的跨组织访问也将被绝对硬隔离阻断。</p>
 </div>
 <button
 ...
 <Globe size={13} className={styles.accentText} />
 <span>{org.name}</span>
 </div>
 ...
 ? `${styles.appBg} ${styles.accentBorder} shadow-md`
 ...
 <div className={`${styles.appBg}/60 border ${styles.cardBorder} p-3.5 rounded-lg flex items-center justify-between`}>
 <div className="space-y-1">
 ...
 <ShieldAlert size={13} className={styles.accentText} />
 <span>成员自主授权列表 (Access Control List - ACL)</span>
 </div>
 <div className={`border ${styles.cardBorder} rounded-lg overflow-hidden ${styles.appBg}/30`}>
 ...
 ? `${styles.appBg} border-pink-500/40 shadow-md`
 : `${styles.cardBg} ${styles.cardBorder} ${styles.sidebarHoverBg}`
 }`}
 >
 <div className={`text-xs font-extrabold ${styles.cardText} flex items-center gap-1.5`}>
 ...
 <div className={`${styles.appBg}/60 border ${styles.cardBorder} p-3.5 rounded-lg space-y-2`}>
 ...
 className={`text-pink-400 hover:text-pink-300 font-bold text-[10px] px-1 ${styles.cardBg}/50 rounded ${styles.sidebarHoverBg} cursor-pointer`}
 >
 + 增授
 </button>
 </div>
 ...
 <div className={`${styles.appBg}/40 border ${styles.cardBorder} rounded-lg p-4 space-y-4`}>
 <div className={`}space-y-0.5">
 <span className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`>
 ...
 <div className={`flex flex-col md:flex-row items-center justify-between gap-4 p-4 ${styles.cardBg}/80 rounded border ${styles.cardBorder} relative`}>
 ...
 {/* Arrow */}
 <div className={`${styles.cardText} font-bold flex items-center justify-center`}>
 <ArrowRight size={14} className="rotate-90 md:rotate-0" />
 </div>

 {/* Node 2 */}
 <div className={`flex-1 text-center p-3 rounded ${styles.appBg} border-2 border-pink-800/40 relative z-10 flex flex-col items-center gap-1 shadow-md pink-950/10`}>
 ...
 * ECOS 安全标准：任何衍生、转化、联结该根源数源的数据项都将自动继承 {currentMark.id}，无法通过普通视图清洗逻辑逃逸。
 ...
 <div className={`flex items-center justify-between text-[9px] border-t ${styles.appBorder} pt-2 ${styles.cardTextMuted}`}>
 <span>届期截至日期:</span>
 ...
 className={`p-1 px-2.5 bg-indigo-900/40 hover:bg-indigo-900 text-[10px] font-bold ${styles.accentText} rounded cursor-pointer`}
 >
 重新激活
 </button>
 ) : (
 ...
 ? '${styles.appBg} ${styles.accentBorder}/50 shadow-md '
 : ${styles.cardBg} ${styles.cardBorder} ${styles.sidebarHoverBg}
 }`}
 >
 ...
 <div className="space-y-3">
 <div className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`>
 <EyeOff size={13} className={`${styles.accentText}`} />
 <span>列级敏感字段遮蔽规则 (Dynamic Column Masking)</span>
 </div>
 <div className={`border ${styles.cardBorder} rounded-lg overflow-hidden ${styles.appBg}/40`}>
 <table className={`w-full text-left text-xs ${styles.cardText}`>
 <thead className={`${styles.cardBg} ${styles.cardTextMuted} text-[10px] font-extrabold uppercase tracking-wider border-b ${styles.cardBorder}`}>
 <tr>
...
<tbody className={`divide-y ${styles.cardBorder} font-mono text-[11px]`}>
 ...
 <tr key={col.column} className={`${styles.sidebarHoverBg}/30`}>
 ...
 <td className="p-3">
 <span className={`h-1.5 w-1.5 inline-block rounded-full mr-1.5 ${col.active && col.maskType !== 'NONE' ? 'bg-emerald-400 animate-pulse' : 'styles.inputBg0'}`} />
 ...
 col.active ? 'bg-emerald-950 text-emerald-400 hover:bg-emerald-900' : '${styles.sidebarActiveBg} ${styles.cardTextMuted} hover:styles.sidebarActiveBg'
 }`}
 >
 {col.active ? '激活生效' : '已关闭'}
 </button>
 ...
 <div className="space-y-3">
 <div className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`>
 <Filter size={13} className={`${styles.accentText}`} />
 <span>行级动态物理隔离策略 (Row-Level Security Policies)</span>
 </div>
 {currentPolicy.rowFilters.length === 0 ? (
 <div className={`text-center p-6 ${styles.appBg}/30 border ${styles.cardBorder} rounded-lg ${styles.cardTextMuted} text-xs`}>
 当前数据集没有添加任何行过滤策略。
 </div>
 ) : (
 <div className="space-y-2">
 {currentPolicy.rowFilters.map((filter, idx) => (
 <div key={idx} className={`${styles.appBg}/60 border ${styles.cardBorder} p-3.5 rounded-lg flex items-center justify-between`}>
 <div className="space-y-1">
 ...
 <div className={`${styles.appBg}/30 border ${styles.cardBorder} rounded-lg p-4 space-y-3.5`}>
 <div className={`flex items-center justify-between border-b ${styles.cardBorder}/60 pb-2`}>
 ...
 <div className={`grid grid-cols-3 gap-2 p-2 rounded ${styles.appBg}/80 ${styles.sidebarHoverBg}`}>
 <span>CA-1209</span>
 ...
 <Activity size={15} className={`${styles.accentText}`} />
...
<tbody className={`divide-y ${styles.cardBorder} font-mono text-[11px]`}>
  {auditLogs.map(log => (
 <tr key={log.id} className={`${styles.sidebarHoverBg}/30 transition-colors`}>
 <td className={`p-3 ${styles.accentText}/80 font-bold`}>{log.timestamp}</td>
 <td className={`p-3 ${styles.cardText}`>{log.username}</td>
 <td className={`p-3 ${styles.cardTextMuted}`}>{log.orgId.split('_')[1] || log.orgId}</td>
 <td className={`p-3 ${styles.cardText} font-semibold`}{log.action}</td>
 <td className={`p-3 ${styles.cardTextMuted}`}>
 <span className={`text-[10px] ${styles.accentText} font-bold ${styles.badgeBg}/20 px-1 py-0.2 rounded border ${styles.accentBorder} mr-1.5`}>
 {log.resourceType}
 </span>
 {log.resourceId}
 </td>
 ...
 <div className={`flex items-center gap-1.5 text-[10px] font-mono ${styles.accentText} ${styles.badgeBg} border ${styles.accentBorder} px-3 py-1 rounded-lg`}>
 ...
 guideSection === 'arch' ? '${styles.accentBg} text-white shadow' : ${styles.cardBg} ${styles.cardTextMuted} ${styles.cardText} ${styles.sidebarHoverBg}
 }`}
 >
 <Shield size={12} />
 <span>核心安全管道机制</span>
 ...
 className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
 guideSection === 'org' ? '${styles.accentBg} text-white shadow' : ${styles.cardBg} ${styles.cardTextMuted} ${styles.cardText} ${styles.sidebarHoverBg}
 }`}
 >
 <Globe size={12} />
 <span>1. 组织初始化隔离</span>
 ...
 className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
 guideSection === 'dac' ? '${styles.accentBg} text-white shadow' : ${styles.cardBg} ${styles.cardTextMuted} ${styles.cardText} ${styles.sidebarHoverBg}
 }`}
 >
 <FolderGit size={12} />
 ...
 className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
 guideSection === 'mac' ? '${styles.accentBg} text-white shadow' : ${styles.cardBg} ${styles.cardTextMuted} ${styles.cardText} ${styles.sidebarHoverBg}
 }`}
 >
 <Tag size={12} />
 ...
 className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
 guideSection === 'pbac' ? '${styles.accentBg} text-white shadow' : ${styles.cardBg} ${styles.cardTextMuted} ${styles.cardText} ${styles.sidebarHoverBg}
 }`}
 >
 <EyeOff size={12} />
 ...
 className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
 guideSection === 'audit' ? '${styles.accentBg} text-white shadow' : ${styles.cardBg} ${styles.cardTextMuted} ${styles.cardText} ${styles.sidebarHoverBg}
 }`}
 >
 <Activity size={12} />
 ...
 <div className={`${styles.appBg} border-l-4 ${styles.accentBorder} p-3.5 rounded-r-lg space-y-1 font-mono text-[10px] ${styles.cardText}`>
 ...
 <li>
 <strong className={`${styles.cardText}`}>逻辑共享隔离</strong>: 允许跨组织数据通过严格授权相互合并，主要在数据集或项目层面施加 ACL 授权，便于集团协同。
 </li>
 <li>
 ...
 <CheckSquare size={12} className={`${styles.cardText}`} />
 <span>初级管理员配置清单</span>
 </div>
 ...
 **任何基于该源表创建的衍生表、视图、过滤导出的数据流资产，都将强制继承并终生沿袭这一安全标记标记锁**。
 ...
 <li>
 <strong className={`${styles.cardText}`}>行过滤 (Row Filter Predicate)</strong>: 注入谓词过滤 SQL，限制物理可见行。例如在行中注入 <code className={`${styles.cardText}`}>"org_id = 'Org_EU_Ops'"</code>，当前用户如果访问此表，所有非欧盟组织范围内的记录将直接从底层物理引擎上蒸发，无法被探测！
 </li>
 </ul>
 </div>
 ...
 <div className={`${styles.appBg} border ${styles.cardBorder} p-4 rounded-xl space-y-3 text-[11px]`}>
 <div className={`${styles.cardText} font-bold border-b ${styles.cardBorder} pb-1 flex items-center gap-1.5`}>
 <Settings size={12} className={`${styles.cardText}`} />
 ...
 <li>
 <strong className={`${styles.cardText}`}>SUCCESS (正常通过)</strong>: 安全网关经链式研判，判定访问动作完全合法，流式渲染并返回经脱敏处理的数据。
 </li>
 <li>
 ...
 <div className="flex items-center gap-3">
 <span className={`p-2 ${styles.badgeBg} ${styles.accentText} rounded-lg border ${styles.accentBorder}`}>
 <Flame size={16} />
 </span>
 <div>
 ...
 <button onClick={() => setShowAddOrgModal(false)} className={`${styles.cardTextMuted} ${styles.cardText}Muted cursor-pointer`}>
 <X size={16} />
 </button>
 </div>
 ...
 className={`w-full ${styles.appBg} border ${styles.cardBorder} rounded p-2 ${styles.cardText} outline-none`}
 />
 </div>
 <div>
 ...
 className={`w-full ${styles.appBg} border ${styles.cardBorder} rounded p-2 ${styles.cardText} font-mono outline-none`}
 />
 </div>
 <div>
 ...
 className={`rounded ${styles.appBg} ${styles.cardBorder}`}
 />
 <label htmlFor="isolation_chk" className={`${styles.cardText} font-semibold cursor-pointer select-none`}>
 强制启用物理域高耸严密隔离
 </label>
 </div>
 <div className={`flex justify-end gap-2 pt-4 border-t ${styles.appBorder}`}>
 <button
 type="button"
 onClick={() => setShowAddOrgModal(false)}
 className={`px-3 py-1.5 ${styles.appBg} ${styles.cardTextMuted} rounded-lg ${styles.sidebarHoverBg} cursor-pointer`}
 >
 取消
 </button>
 <button
 ...
 <button onClick={() => setShowAddMemberModal(false)} className={`${styles.cardTextMuted} ${styles.cardText}Muted cursor-pointer`}>
 <X size={16} />
 </button>
 </div>
 ...
 <button onClick={() => setShowAddMarkingModal(false)} className={`${styles.cardTextMuted} ${styles.cardText}Muted cursor-pointer`}>
 <X size={16} />
 </button>
 </div>
 ...
 className={`w-full ${styles.appBg} border ${styles.cardBorder} rounded p-2 ${styles.cardText} outline-none h-16 resize-none`}
 />
 </div>
 <div className={`flex justify-end gap-2 pt-4 border-t ${styles.appBorder}`}>
 <button
 ...
 <button onClick={() => setShowAddPurposeModal(false)} className={`${styles.cardTextMuted} ${styles.cardText}Muted cursor-pointer`}>
 <X size={16} />
 </button>
 </div>
 ...
 <button onClick={() => setShowDiagnosticModal(false)} className={`${styles.cardTextMuted} ${styles.cardText} cursor-pointer p-1 ${styles.appBg} ${styles.sidebarHoverBg} rounded`}>
 <X size={16} />
 </button>
 </div>
 ...
 let textColor = 'styles.cardTextMuted';
 let icon = 'Circle';
 ...
 return (
 <div key={idx} className={`ml-4 pl-3 border-l ${styles.accentBorder} py-1 ${styles.appBg}/30 rounded-r text-[11px] font-mono flex flex-col gap-1`}>
 ...
 return (
 <div key={idx} className={`ml-10 text-[10px] ${styles.cardTextMuted} italic ${styles.cardBg}/40 p-1.5 rounded border ${styles.appBorder}`}>
 {line}
 </div>
 );
 }
 ...
 className={`px-4 py-1.5 ${styles.appBg} ${styles.cardText} font-black rounded-lg ${styles.sidebarHoverBg} cursor-pointer border ${styles.cardBorder} transition-all text-xs`}
 >
 关闭诊断书
 </button>
 </div>
