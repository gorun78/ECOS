...
export default function OntologyDesigner() {
  ...
  return (
    <div className={`flex flex-col lg:flex-row h-full ${styles.appBg} font-sans ${styles.appText}`}>
      {toast && (
        ...
        <div className="flex-1 overflow-y-auto">
          {entities.map(e => (
            <div key={e.id} onClick={() => setSelectedEntityId(e.id)}
              className={`px-3 py-2.5 border-b ${styles.divider} cursor-pointer transition flex items-center gap-2.5 ${
                selectedEntityId === e.id ? "bg-indigo-50 border-l-2 border-l-indigo-500" : `${styles.sidebarHoverBg}`
              }`}>
              <Database className={`w-3.5 h-3.5 ${e.entityType === "MASTER" ? "text-amber-500" : "text-blue-500"} shrink-0`} />
              <div className="min-w-0 flex-1">
                <div className={`text-xs font-semibold ${styles.cardText} truncate`}>{e.code}</div>
                <div className={`text-[10px] ${styles.cardTextMuted} truncate`}>{e.name} · {entTypeLabel(e.entityType)}</div>
              </div>
              <span className={`text-[9px] ${styles.muted} opacity-60 font-mono`}>{properties[e.id]?.length || 0}p</span>
            </div>
          ))}
        </div>
      </div>
        ...
        {!selectedEntity ? (
          <div className={`flex-1 flex items-center justify-center text-xs ${styles.cardTextMuted}`}>
            <div className="text-center">
              <Globe className={`w-8 h-8 mx-auto mb-2 ${styles.muted} opacity-50`} />
          ...
          <>
            ...
            <div className={`p-4 border-b ${styles.cardBorder}`}>
              <div className="flex items-center justify-between mb-3">
                <h3 className={`text-sm font-bold ${styles.appText} flex items-center gap-2`}>
                  <Database className={`w-4 h-4 ${selectedEntity!.entityType === "MASTER" ? "text-amber-500" : "text-blue-500"}`} />
                  {selectedEntity!.code}
                </h3>
                <div className="flex gap-1">
                  <button onClick={() => setEditEntity({ ...selectedEntity! })} className={`p-1.5 hover:opacity-70 rounded ${styles.cardTextMuted} transition`} title={t("ontology.designer.editEntity")}>
              ...
              {editEntity && editEntity.id === selectedEntityId ? (
                <div className={`space-y-2 ${styles.appBg} rounded-lg p-3`}>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setEditEntity(null)} className={`px-3 py-1.5 ${styles.appBg} ${styles.sidebarHoverBg} ${styles.cardTextMuted} rounded text-xs transition`}>{t("ontology.designer.cancel")}</button>
                  </div>
                </div>
              ) : (
                <div className={`text-[11px] ${styles.cardText} space-y-0.5`}>
                  <div><span className={styles.cardTextMuted}>{t("ontology.designer.label.name")}</span>{selectedEntity!.name}</div>
                  <div><span className={styles.cardTextMuted}>{t("ontology.designer.label.type")}</span>{entTypeLabel(selectedEntity!.entityType)}</div>
                  {selectedEntity!.description && <div><span className={styles.cardTextMuted}>{t("ontology.designer.label.description")}</span>{selectedEntity!.description}</div>}
                </div>
              )}
            </div>
            ...
            <div className={`p-4 border-b ${styles.cardBorder}`}>
              <div className="flex items-center justify-between mb-3">
                <h4 className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`}>
              ...
              {showAddProp && (
                <div className="bg-indigo-50 rounded-lg p-3 mb-3 space-y-2">
                  ...
                  <div className="flex items-center gap-4">
                    <label className={`flex items-center gap-1 text-[10px] ${styles.cardText}`}><input type="checkbox" checked={newProp.requiredFlag === 1} onChange={e => setNewProp(p => ({...p, requiredFlag: e.target.checked ? 1 : 0}))} />{t("ontology.designer.required")}</label>
                    <label className={`flex items-center gap-1 text-[10px] ${styles.cardText}`}><input type="checkbox" checked={newProp.searchableFlag === 1} onChange={e => setNewProp(p => ({...p, searchableFlag: e.target.checked ? 1 : 0}))} />{t("ontology.designer.searchable")}</label>
                  </div>
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setShowAddProp(false)} className={`px-3 py-1.5 ${styles.appBg} ${styles.sidebarHoverBg} ${styles.cardTextMuted} rounded text-xs`}>{t("ontology.designer.cancel")}</button>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1">
                {selectedProps.map(prop => (
                  <div key={prop.id} className={`group flex items-center gap-2 px-2 py-1.5 rounded ${styles.sidebarHoverBg}`}>
                    {editingProp?.id === prop.id ? (
                      <div className="flex-1 space-y-1">
                        ...
                        <div className="flex gap-1">
                          ...
                          <button onClick={() => setEditingProp(null)} className={`text-[10px] ${styles.appBg} ${styles.sidebarHoverBg} ${styles.cardTextMuted} rounded px-2 py-0.5`}>{t("ontology.designer.cancel")}</button>
                        </div>
                      </div>
                    ) : (
                      <>
                        <span className={`text-[10px] font-mono font-semibold ${styles.cardText} w-20 truncate`}>{prop.code}</span>
                        <span className={`text-[10px] ${styles.cardTextMuted} flex-1 truncate`}>{prop.name || prop.code}</span>
                        <span className={`text-[9px] font-mono ${styles.cardTextMuted} ${styles.sidebarBg} rounded px-1.5 py-0.5`}>{prop.propertyType === "FUNCTION" ? `${prop.propertyType}:${prop.functionType || "?"}` : prop.propertyType}</span>
                        {prop.requiredFlag === 1 && <span className="text-[9px] text-red-400 font-bold">*</span>}
                        <button onClick={() => setEditingProp({...prop})} className={`opacity-0 group-hover:opacity-100 p-0.5 ${styles.sidebarHoverBg} rounded ${styles.cardTextMuted}`}><Edit3 className="w-3 h-3" /></button>
                ...
                {selectedProps.length === 0 && <div className={`text-[10px] ${styles.cardTextMuted} text-center py-4`}>{t("ontology.designer.noProperties")}</div>}
              </div>
            </div>
            ...
            <div className="p-4">
              ...
              {showAddRel && (
                <div className="bg-indigo-50 rounded-lg p-3 mb-3 space-y-2">
                  ...
                  <div className="flex gap-2">
                    ...
                    <button onClick={() => setShowAddRel(false)} className={`px-3 py-1.5 ${styles.appBg} ${styles.sidebarHoverBg} ${styles.cardTextMuted} rounded text-xs`}>{t("ontology.designer.cancel")}</button>
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                {selectedRels.map(rel => {
                  ...
                  return (
                    <div key={rel.id} className={`group flex items-center gap-2 px-2 py-1.5 rounded ${styles.sidebarHoverBg}`}>
                      ...
                      <div className="flex-1 min-w-0">
                        <div className={`text-[10px] font-semibold ${styles.cardText}`}>{rel.code}</div>
                        <div className={`text-[9px] ${styles.cardTextMuted}`}>
                          {isSource ? "→" : "←"} {otherEntity?.code || otherId?.slice(0,8)}
                          <span className={`ml-1 font-mono ${styles.muted} opacity-60`}>[{rel.relationshipType}]</span>
                        </div>
                      </div>
                ...
                {selectedRels.length === 0 && <div className={`text-[10px] ${styles.cardTextMuted} text-center py-4`}>{t("ontology.designer.noRelations")}</div>}
              </div>
            </div>
          </>
        )}
