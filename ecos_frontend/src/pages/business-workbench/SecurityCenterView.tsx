...
export interface SecurityMarking {
  ...
  displayName: string;
  apiName: string;
  classificationLevel: 'CONFIDENTIAL' | 'SECRET' | 'TOP_SECRET' | 'RESTRICTED';
...
const gdprSteps: PlaybookStep[] = [
  {
    ...
    tab: 'orgs',
    description: '作为 Foundry 最外层的安全边界，首先建立欧盟运营分部组织。此边界是严密物理隔离，非特许共享的跨组织访问将被硬件阻断。',
    foundryContext: '【官方设计】Foundry Organization 用于在多租户/多分支机构中实现严格数据壁垒，是零信任的最底层基石。',
  ...
  },
  {
    ...
    foundryContext: '【官方设计】DAC (Discretionary Access Control) 为项目提供细粒度的 ACL（Owner, Editor, Viewer, Discoverer）控制。',
  ...
  },
  {
    ...
    tab: 'mac',
    description: '向核心旅客表 ds_passenger_manifest 应用「GDPR_PII」强制安全标记。在 Foundry 中，下游的所有派生表都会自动、无条件地继承此标记锁。',
    foundryContext: '【官方设计】Markings (MAC) 具有极其强大的级联沿袭传播（Propagation）特征，数据加工无法逃逸标记锁定。',
  ...
  },
  {
    ...
    foundryContext: '【官方设计】Purpose (PBAC) 控制数据的正当商业合规使用背景，它是解密 Markings 标记锁的唯一合规通道。',
  ...
  },
  {
    ...
    foundryContext: '【官方设计】在零信任网关中，所有访问必须满足：Org白名单 ➔ Project DAC ➔ Marking (MAC) ➔ Purpose (PBAC) 的链式全部通过。',
...
const financeSteps: PlaybookStep[] = [
  {
    ...
    foundryContext: '【官方设计】Organization 支持设置 IP 白名单 CIDR，未在物理域段内的请求会被直接阻断。',
  ...
  },
  {
    ...
    foundryContext: '【官方设计】Discoverable 可选属性。如果未打开，非项目成员甚至无法通过索引检索到该项目文件夹的存在。',
  ...
  },
  {
    ...
    foundryContext: '【官方设计】Markings 是超越文件权限的全局强制标记。拥有该项目 Editor 权限，若无此安全标记亦不可读原始数据。',
  ...
  },
  {
    ...
    foundryContext: '【官方设计】Foundry PBAC 用途有着极其严格的生命周期。一经到期或管理员手动吊销，底层解密钥匙自动失活。',
  ...
  },
  {
    ...
    foundryContext: '【官方设计】行列级安全策略（Row/Column Level Security）是附加在解密后的数据渲染层的，根据当前用户身份动态决定列脱敏或行过滤。',
...
export default function SecurityCenterView({
  ...
  const [newMarkLevel, setNewMarkLevel] = useState<'CONFIDENTIAL' | 'SECRET' | 'TOP_SECRET' | 'RESTRICTED'>('CONFIDENTIAL');
  ...
  const handleRunSimulation = async () => {
    ...
    try {
      ...
      const response = await fetch('/api/v1/security/decrypt', {
        method: 'POST',
        headers: {
  ...
  return (
    <div className={`h-full w-full flex ${styles.appBg} ${styles.appText} overflow-hidden font-sans select-none`}>
      ...
      <div className={`w-64 ${styles.sidebarBg} border-r ${styles.sidebarBorder} flex flex-col justify-between shrink-0`}>
        <div className="p-4 space-y-5">
          <div className="flex items-center gap-2.5 px-2">
            ...
            <div>
              <div className={`text-xs font-black tracking-wider ${styles.cardText} uppercase`}>Foundry 安全中心</div>
              <div className={`text-[9px] ${styles.accentText} font-mono`}>SECURE POSTURE CONTROL</div>
            </div>
          </div>

          <div className={`h-px ${styles.cardBorder}`} />
          ...
          <nav className="space-y-1">
            ...
            <button
              ...
              className={`w-full flex items-center gap-2.5 px-3 py-2 text-xs rounded-lg transition-all font-semibold ${
                activeTab === 'overview'
                  ...
                  : `${styles.sidebarText} ${styles.sidebarHoverBg} hover:${styles.cardText}`
              }`}
            >
              <LucideIcon name="Shield" size={13} className={activeTab === 'overview' ? styles.accentText : styles.cardTextMuted} />
              <span>安全合规概览</span>
            </button>
            ...
            >
              <LucideIcon name="Globe" size={13} className={activeTab === 'orgs' ? styles.accentText : styles.cardTextMuted} />
            ...
            >
              <LucideIcon name="FolderGit" size={13} className={activeTab === 'dac' ? styles.accentText : styles.cardTextMuted} />
            ...
            >
              <LucideIcon name="Tag" size={13} className={activeTab === 'mac' ? styles.accentText : styles.cardTextMuted} />
            ...
            >
              <LucideIcon name="ClipboardList" size={13} className={activeTab === 'pbac' ? styles.accentText : styles.cardTextMuted} />
            ...
            >
              <LucideIcon name="EyeOff" size={13} className={activeTab === 'row_col' ? styles.accentText : styles.cardTextMuted} />
              <span>行列级安全策略</span>
            </button>
            ...
            >
              <LucideIcon name="Activity" size={13} className={activeTab === 'audit' ? styles.accentText : styles.cardTextMuted} />
            ...
            >
              <LucideIcon name="BookOpen" size={13} className={activeTab === 'guide' ? styles.accentText : styles.cardTextMuted} />
              <span>管理员操作手册</span>
            </button>
          </nav>
        </div>

        <div className={`p-4 space-y-2 ${styles.sidebarBg} border-t ${styles.sidebarBorder}`}>
      ...
      <div className="flex-1 flex flex-col overflow-hidden">
        ...
        {/* VIEW TITLE BAR */}
        <div className={`h-14 ${styles.sidebarBg} border-b ${styles.sidebarBorder} px-6 flex items-center justify-between shrink-0`}>
          <div className="flex items-center gap-3">
            <div className={`text-sm font-black ${styles.cardText} flex items-center gap-2`}>
              <span>Palantir Security Guardian</span>
              <span className={`text-[10px] ${styles.cardBg} ${styles.cardTextMuted} px-2 py-0.5 rounded font-mono uppercase`}>
                Enterprise Shield
              </span>
            </div>
          </div>
        ...
        <div className={`flex-1 overflow-y-auto p-6 ${styles.appBg}`}>
          ...
          {activeTab === 'overview' && (
            <div className="space-y-6">
              ...
              <div className="grid grid-cols-1 md:grid-cols-4 gap-4">
                <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-4 flex items-center justify-between`}>
                  <div className="space-y-1">
                    <div className={`text-[11px] ${styles.cardTextMuted} font-semibold uppercase`}>安全隔离域 (Orgs)</div>
                    <div className={`text-xl font-mono font-black ${styles.cardText}`}>{orgs.length}</div>
                  </div>
                ...
                <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-4 flex items-center justify-between`}>
                  <div className="space-y-1">
                    <div className={`text-[11px] ${styles.cardTextMuted} font-semibold uppercase`}>受控工程项目 (DAC)</div>
                    <div className={`text-xl font-mono font-black ${styles.cardText}`}>{projects.length}</div>
                  </div>
                  <span className="p-2 rounded-lg bg-blue-500/10 text-blue-400">
                    <LucideIcon name="FolderGit" size={18} />
                  </span>
                </div>
                <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-4 flex items-center justify-between`}>
                  <div className="space-y-1">
                    <div className={`text-[11px] ${styles.cardTextMuted} font-semibold uppercase`}>强制密级标记 (MAC)</div>
                    <div className={`text-xl font-mono font-black ${styles.cardText}`}>{markings.length}</div>
                  </div>
                ...
                <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-4 flex items-center justify-between`}>
                  <div className="space-y-1">
                    <div className={`text-[11px] ${styles.cardTextMuted} font-semibold uppercase`}>合规目的管控 (PBAC)</div>
                    <div className={`text-xl font-mono font-black ${styles.cardText}`}>
                      {purposes.filter(p => p.status === 'ACTIVE').length} <span className={`text-xs ${styles.cardTextMuted}`}>/ {purposes.length}</span>
                    </div>
                  </div>
              ...
              <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-4 lg:col-span-2 space-y-4`}>
                  <div className="flex items-center justify-between">
                    <span className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`}>
                      <LucideIcon name="TrendingUp" size={13} className={styles.accentText} />
                      安全策略鉴权评估趋势 (Policy Evaluation Trend)
                    </span>
                    <span className={`text-[10px] ${styles.cardTextMuted}`}>过去7天动态安全阻断详情</span>
                  </div>
                  <div className="h-64">
                ...
                <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-4 space-y-4 flex flex-col justify-between`}>
                  <div className="space-y-1">
                    <div className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`}>
                      <LucideIcon name="PieChart" size={13} className={styles.accentText} />
                      安全标记保护下的高敏数据分布 (MAC Tag Distribution)
                    </div>
                    <p className={`text-[10px] ${styles.cardTextMuted}`}>按保密分类所涵盖的核心数源占比</p>
                  </div>
                  <div className="h-44 flex items-center justify-center relative">
                    ...
                    <div className="absolute flex flex-col items-center">
                      <span className={`text-xs font-bold font-mono ${styles.cardTextMuted}`}>受护项</span>
                      <span className={`text-lg font-black font-mono ${styles.cardText}`}>100%</span>
                    </div>
                  </div>
              ...
              <div className={`${styles.cardBg} border-2 ${styles.accentBorder} rounded-xl p-5 space-y-4`}>
                <div className={`flex items-center justify-between border-b ${styles.cardBorder} pb-3`}>
                  <div className="space-y-0.5">
                    <span className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`}>
                      <LucideIcon name="Zap" size={14} className={`${styles.accentText} animate-pulse`} />
                      零信任策略仿真演算器 (Zero-Trust Security Policy Simulator)
                    </span>
                    <p className={`text-[10px] ${styles.cardTextMuted}`}>模拟不同用户、敏感资产和具体分析目的在 Foundry 级联策略链条中的真实决策判定结果</p>
                  </div>
                  <button
                    onClick={handleRunSimulation}
                    className={`px-4 py-1.5 ${styles.accentBg} ${styles.accentHover} text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer`}
                  >
                ...
                <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
                  ...
                  {/* Inputs */}
                  <div className={`space-y-3 ${styles.appBg} p-3.5 rounded-lg border ${styles.cardBorder}`}>
                    <div>
                      <label className={`text-[10px] ${styles.cardTextMuted} font-bold block mb-1`}>模拟主体 (Subject User)</label>
                      <select
                        value={simUser}
                        onChange={(e) => setSimUser(e.target.value)}
                        className={`w-full ${styles.inputBg} border ${styles.inputBorder} rounded px-2.5 py-1.5 text-xs ${styles.inputText} font-mono outline-none`}
                      >
                    ...
                    <div>
                      <label className={`text-[10px] ${styles.cardTextMuted} font-bold block mb-1`}>敏感资源 (Sensitive Dataset)</label>
                      <select
                        value={simDataset}
                    ...
                    <div>
                      <label className={`text-[10px] ${styles.cardTextMuted} font-bold block mb-1`}>使用目的环境 (Purpose Context)</label>
                      <select
                        value={simPurpose}
                    ...
                    <div>
                      <label className={`text-[10px] ${styles.cardTextMuted} font-bold block mb-1`}>物理客户端 IP (Client IP - Whitelist CIDR)</label>
                      <input
                        type="text"
                        value={simIp}
                        onChange={(e) => setSimIp(e.target.value)}
                        placeholder="例: 10.120.5.23"
                        className={`w-full ${styles.inputBg} border ${styles.inputBorder} rounded px-2.5 py-1.5 text-xs ${styles.inputText} font-mono outline-none focus:${styles.accentBorder.replace('border-', 'border-')}`}
                      />
                      <span className={`text-[8px] ${styles.cardTextMuted} mt-1 block`}>
                  ...
                  {/* Verdict & Details */}
                  <div className={`md:col-span-2 ${styles.appBg} p-3.5 rounded-lg border ${styles.cardBorder} flex flex-col justify-between`}>
                    {simResult ? (
                      <div className="space-y-3">
                        <div className="flex items-center justify-between">
                          <span className={`text-[10px] ${styles.cardTextMuted} font-bold`}>校验结果反馈 (Decision Result)</span>
                        ...
                        <div className={`space-y-1.5 font-mono text-[10px] ${styles.cardText} overflow-y-auto max-h-32 pr-2`}>
                      ...
                      <div className={`flex-1 flex flex-col items-center justify-center ${styles.cardTextMuted} space-y-2 py-6`}>
                        <LucideIcon name="ShieldAlert" size={24} className={styles.muted} />
                        <span className="text-xs">等待仿真触发，请点击右上角「一键仿真演算」按钮</span>
                      </div>
                    )}

                    <div className={`text-[9px] ${styles.cardTextMuted} border-t ${styles.cardBorder} pt-2 flex items-center gap-1.5`}>
                      <LucideIcon name="Info" size={10} />
                      <span>本仿真完全模拟 Palantir Foundry 零信任架构下的 <strong>组织隔离 ➔ DAC ➔ MAC ➔ 目的链(PBAC)</strong> 四重复合门禁。</span>
                    </div>
                  </div>
                </div>
              </div>
          ...
          {activeTab === 'orgs' && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className={`text-sm font-black ${styles.cardText}`}>Foundry 组织边界配置 (Organizations Setup)</h3>
                  <p className={`text-[10px] ${styles.cardTextMuted}`}>Foundry 最高级别安全隔离机制。即使授予了文件权限，非共享协议白名单下的跨组织访问也将被绝对硬隔离阻断。</p>
                </div>
                <button
                  onClick={() => setShowAddOrgModal(true)}
                   className={`px-3 py-1.5 ${styles.accentBg} ${styles.accentHover} text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1 cursor-pointer`}
                >
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {orgs.map(org => (
                  <div key={org.id} className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-5 space-y-4`}>
                    <div className="flex items-start justify-between">
                      <div className="space-y-0.5">
                        <div className={`text-xs font-extrabold ${styles.cardText} flex items-center gap-1.5`}>
                          <LucideIcon name="Globe" size={13} className={styles.accentText} />
                          <span>{org.name}</span>
                        </div>
                        <div className={`text-[10px] ${styles.cardTextMuted} font-mono`}>ID: {org.id}</div>
                      </div>
                    ...
                    <div className={`h-px ${styles.appBg}`} />

                    <div className={`space-y-2 text-xs ${styles.cardText}`}>
                      <div className="flex justify-between items-center">
                        <span className={styles.cardTextMuted}>组织活跃成员:</span>
                        <span className={`font-mono font-bold ${styles.cardText}`}>{org.memberCount} 人</span>
                      </div>
                      <div className="flex justify-between items-center">
                        <span className={styles.cardTextMuted}>已授权跨组织共享:</span>
                        <span className={`font-mono text-[10px] ${styles.accentText}`}>
                          {org.crossOrgSharing.length > 0 ? org.crossOrgSharing.join(', ') : '无共享'}
                        </span>
                      </div>
                      <div className="space-y-1">
                        <div className={styles.cardTextMuted}>活跃白名单IP段 (CIDR):</div>
                        <div className="flex flex-wrap gap-1">
                          {org.ipRanges.map(ip => (
                            <span key={ip} className={`${styles.appBg} border ${styles.cardBorder} px-1.5 py-0.5 rounded text-[10px] font-mono ${styles.accentText}`}>
                              {ip}
                            </span>
                          ))}
                        </div>
                    ...
                    <div className="flex items-center justify-end gap-1.5 pt-2">
                      <button 
                        ...
                        className={`p-1 ${styles.appBg} ${styles.sidebarHoverBg} rounded border ${styles.cardBorder} text-[10px] font-bold ${styles.cardText} cursor-pointer`}
                      >
                        追加网段
                      </button>
                      <button 
            ...
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-4 space-y-4`}>
                <div className={`flex items-center justify-between border-b ${styles.cardBorder} pb-2`}>
                  <span className={`text-xs font-bold ${styles.cardText}`}>受控安全工程项目</span>
                  <span className={`text-[9px] font-mono ${styles.cardTextMuted}`}>DAC CONTROLS</span>
                </div>
                    ...
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg border transition-all flex flex-col gap-1 cursor-pointer ${
                        selectedProjectId === proj.id
                          ? `${styles.appBg} ${styles.accentBorder} shadow-md shadow-indigo-500/5`
                          : `${styles.cardBg} ${styles.cardBorder} ${styles.sidebarHoverBg}`
                      }`}
                    >
                      <div className={`text-xs font-extrabold ${styles.cardText} truncate`}>{proj.name}</div>
                      <p className={`text-[10px] ${styles.cardTextMuted} line-clamp-1`}>{proj.description}</p>
                      <div className={`flex items-center gap-1.5 mt-1 text-[9px] ${styles.cardTextMuted}`}>
                        <LucideIcon name="Users" size={10} />
                        <span>授权成员: {proj.members.length}</span>
                        <span>•</span>
                         <span className={proj.autoPropagation ? 'text-emerald-400' : styles.cardTextMuted}>
              ...
              <div className={`lg:col-span-2 ${styles.cardBg} border ${styles.cardBorder} rounded-xl p-5 space-y-6`}>
                {(() => {
                  const currentProj = projects.find(p => p.id === selectedProjectId);
                  if (!currentProj) return <div className={`${styles.cardTextMuted} text-xs`}>请在左侧选择项目</div>;

                  return (
                    <div className="space-y-6">
                       <div className={`flex items-start justify-between border-b ${styles.cardBorder} pb-4`}>
                         <div className="space-y-1">
                           <h3 className={`text-sm font-black ${styles.cardText}`}>{currentProj.name}</h3>
                           <p className={`text-xs ${styles.cardTextMuted} leading-relaxed`}>{currentProj.description}</p>
                        </div>
                        <button
                          onClick={() => setShowAddMemberModal(true)}
                           className={`px-3 py-1.5 ${styles.accentBg} ${styles.accentHover} text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1 shrink-0 cursor-pointer`}
                        >
                      ...
                      <div className="grid grid-cols-2 gap-4">
                         <div className={`${styles.appBg} border ${styles.cardBorder} p-3.5 rounded-lg flex items-center justify-between`}>
                           <div className="space-y-1">
                             <div className={`text-[10px] ${styles.cardTextMuted} font-bold`}>跨组织可见性 (Discoverability)</div>
                             <div className={`text-xs ${styles.cardText}`}>
                          ...
                          <button
                            ...
                            className={`p-1 px-2.5 rounded text-[10px] font-black cursor-pointer ${
                               currentProj.discoverableAllOrgs ? `${styles.badgeBg} ${styles.accentText}` : `${styles.sidebarActiveBg} ${styles.cardTextMuted}`
                            }`}
                          >
                            切换可见性
                          </button>
                         ...
                         <div className={`${styles.appBg} border ${styles.cardBorder} p-3.5 rounded-lg flex items-center justify-between`}>
                           <div className="space-y-1">
                             <div className={`text-[10px] ${styles.cardTextMuted} font-bold`}>继承传播锁 (Auto-Inheritance)</div>
                             <div className={`text-xs ${styles.cardText}`}>
                          ...
                          <button
                            ...
                            className={`p-1 px-2.5 rounded text-[10px] font-black cursor-pointer ${
                               currentProj.autoPropagation ? `${styles.badgeBg} ${styles.accentText}` : `${styles.sidebarActiveBg} ${styles.cardTextMuted}`
                            }`}
                          >
                            切换沿袭
                          </button>
                      ...
                      {/* Members Table */}
                      <div className="space-y-3">
                         <div className={`text-xs font-bold ${styles.cardText} flex items-center gap-1`}>
                           <LucideIcon name="ShieldAlert" size={13} className={styles.accentText} />
                          <span>成员自主授权列表 (Access Control List - ACL)</span>
                        </div>
                         <div className={`border ${styles.cardBorder} rounded-lg overflow-hidden ${styles.appBg}`}>
                           <table className={`w-full text-left text-xs ${styles.cardText}`}>
                             <thead className={`${styles.cardBg} ${styles.cardTextMuted} text-[10px] font-extrabold uppercase tracking-wider border-b ${styles.cardBorder}`}>
                              <tr>
                             ...
                             <tbody className={`divide-y ${styles.cardBorder} font-mono`}>
                              {currentProj.members.map(member => (
                                 <tr key={member.username} className={`${styles.sidebarHoverBg}`}>
                                   <td className={`p-3 font-semibold ${styles.cardText}`}>{member.username}</td>
                                  <td className="p-3">
                                    <span className={`px-2 py-0.5 rounded text-[9px] font-extrabold ${
                                      ...
                                      member.role === 'Editor' ? 'bg-blue-950 text-blue-400 border border-blue-900/30' :
                                       ...
                                       `${styles.sidebarActiveBg} ${styles.cardTextMuted}`
                                    }`}>
                                      {member.role}
                                    </span>
                                  </td>
                                   <td className={`p-3 ${styles.cardTextMuted}`}>{member.grantedBy}</td>
                                   <td className={`p-3 ${styles.cardTextMuted}`}>{member.grantedAt}</td>
          ...
          {activeTab === 'mac' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-4 space-y-4`}>
                <div className={`flex items-center justify-between border-b ${styles.cardBorder} pb-2`}>
                  <span className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`}>
                    <LucideIcon name="Tag" size={13} className="text-pink-400" />
                    安全标记 (MAC Markings)
                  </span>
                  <button
                    onClick={() => setShowAddMarkingModal(true)}
                    className={`p-1 ${styles.accentBg} ${styles.accentHover} text-white rounded font-bold text-[10px] cursor-pointer`}
                  >
                    + 新建标记
                  </button>
                </div>
                    ...
                    <button
                      ...
                      className={`w-full text-left p-3 rounded-lg border transition-all flex flex-col gap-1 cursor-pointer ${
                        selectedMarkingId === m.id
                          ? `${styles.appBg} border-pink-500/40 shadow-md shadow-pink-500/5`
                          : `${styles.cardBg} ${styles.cardBorder} ${styles.sidebarHoverBg}`
                      }`}
                    >
                      <div className={`text-xs font-extrabold ${styles.cardText} flex items-center gap-1.5`}>
                      ...
                      <p className={`text-[10px] ${styles.cardTextMuted} line-clamp-1`}>{m.description}</p>
                      <div className={`flex items-center gap-2 mt-1.5 text-[9px] ${styles.cardTextMuted}`}>
                        <span className={`${styles.sidebarActiveBg} px-1 py-0.2 rounded font-mono ${styles.cardText}`}>{m.classificationLevel}</span>
              ...
              <div className={`lg:col-span-2 ${styles.cardBg} border ${styles.cardBorder} rounded-xl p-5 space-y-6`}>
                {(() => {
                  ...
                  if (!currentMark) return <div className={`${styles.cardTextMuted} text-xs`}>请选择一个安全标记</div>;

                  return (
                    <div className="space-y-6">
                      
                       <div className={`border-b ${styles.cardBorder} pb-4 space-y-1.5`}>
                         <div className="flex items-center justify-between">
                           ...
                           <span className={`text-[10px] ${styles.cardTextMuted}`}>API Key: {currentMark.apiName}</span>
                         </div>
                         <h3 className={`text-sm font-black ${styles.cardText}`}>{currentMark.displayName}</h3>
                         <p className={`text-xs ${styles.cardTextMuted} leading-relaxed`}>{currentMark.description}</p>
                      </div>

                      {/* Detail attributes */}
                      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                         <div className={`${styles.appBg} border ${styles.cardBorder} p-3.5 rounded-lg space-y-2`}>
                           <span className={`text-[10px] ${styles.cardTextMuted} font-bold block`}>获授此标记的用户组 (Authorized Recipient Groups)</span>
                          <div className="flex flex-wrap gap-1.5 pt-1">
                            {currentMark.grantedGroups.map(grp => (
                               <span key={grp} className={`${styles.cardBg} border ${styles.cardBorder} px-2 py-0.5 rounded text-[10px] font-mono text-pink-300`}>
                                {grp}
                              </span>
                            ))}
                            <button
                              onClick={() => {
                               ...
                               className={`text-pink-400 hover:text-pink-300 font-bold text-[10px] px-1 ${styles.cardBg} rounded ${styles.sidebarHoverBg} cursor-pointer`}
                            >
                              + 增授
                            </button>
                          </div>
                         ...
                         <div className={`${styles.appBg} border ${styles.cardBorder} p-3.5 rounded-lg space-y-2`}>
                           <span className={`text-[10px] ${styles.cardTextMuted} font-bold block`}>当前锚定的根源数据集 (Anchored Root Datasets)</span>
                          <div className="flex flex-wrap gap-1.5 pt-1">
                            {currentMark.appliedDatasets.map(ds => (
                               <span key={ds} className={`${styles.cardBg} border ${styles.cardBorder} px-2 py-0.5 rounded text-[10px] font-mono ${styles.cardText}`}>
                                {ds}
                              </span>
                            ))}
                          </div>
                      ...
                      {/* Lineage Propagation simulator graph */}
                       <div className={`${styles.appBg} border ${styles.cardBorder} rounded-lg p-4 space-y-4`}>
                         <div className="space-y-0.5">
                           <span className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`}>
                             <LucideIcon name="Workflow" size={13} className="text-pink-400" />
                             Palantir 沿袭安全级联传递图 (Marking Propagation & Lineage Tracker)
                           </span>
                           <p className={`text-[10px] ${styles.cardTextMuted}`}>展现标记由于向下衍生转换，从而在底层管道中对下游节点产生的自动化锁定传染效果</p>
                        </div>

                         <div className={`flex flex-col md:flex-row items-center justify-between gap-4 p-4 ${styles.cardBg} rounded border ${styles.cardBorder} relative`}>
                          
                          {/* Node 1 */}
                           <div className={`flex-1 text-center p-3 rounded ${styles.appBg} border-2 ${styles.cardBorder} relative z-10 flex flex-col items-center gap-1`}>
                             <LucideIcon name="Database" size={16} className={styles.cardTextMuted} />
                             <span className={`text-[10px] font-bold ${styles.cardText}`}>根源数据集</span>
                             <span className={`text-[8px] font-mono ${styles.cardTextMuted}`}>ds_aviation_raw</span>
                            <div className="mt-1.5 flex items-center gap-1 text-[8px] bg-pink-950/60 text-pink-400 border border-pink-900/30 px-1 rounded">
                              <LucideIcon name="Tag" size={8} />
                              <span>已打标锁</span>
                            </div>
                          </div>

                          {/* Arrow */}
                           <div className={`${styles.cardTextMuted} font-bold flex items-center justify-center`}>
                            <LucideIcon name="ArrowRight" size={14} className="rotate-90 md:rotate-0" />
                          </div>

                          {/* Node 2 */}
                           <div className={`flex-1 text-center p-3 rounded ${styles.appBg} border-2 border-pink-800/40 relative z-10 flex flex-col items-center gap-1 shadow-md shadow-pink-950/10`}>
                             <LucideIcon name="Cpu" size={16} className="text-pink-400" />
                             <span className={`text-[10px] font-bold ${styles.cardText}`}>清洗同步管道</span>
                             <span className={`text-[8px] font-mono ${styles.cardTextMuted}`}>sync_aviation_cleansed</span>
                          ...
                          {/* Node 3 */}
                           <div className={`flex-1 text-center p-3 rounded ${styles.appBg} border-2 border-pink-700/60 relative z-10 flex flex-col items-center gap-1 shadow-lg shadow-pink-950/20`}>
                             <LucideIcon name="Workflow" size={16} className="text-pink-400 animate-pulse" />
                             <span className={`text-[10px] font-bold ${styles.cardText}`}>派生本体对象</span>
                             <span className={`text-[8px] font-mono ${styles.cardTextMuted}`}>Aviation_Object_Derived</span>
                            <div className="mt-1.5 flex items-center gap-1 text-[8px] bg-red-950 text-red-400 border border-red-900/30 px-1 rounded font-black">
                              <LucideIcon name="ShieldAlert" size={8} />
                              <span>终点级联锁定</span>
                            </div>
                          </div>
                        </div>

                         <p className={`text-[9px] ${styles.cardTextMuted} text-center italic`}>
                          * Foundry 安全标准：任何衍生、转化、联结该根源数源的数据项都将自动继承 {currentMark.id}，无法通过普通视图清洗逻辑逃逸。
                        </p>
                      </div>
                    </div>
                  );
          ...
          {activeTab === 'pbac' && (
            <div className="space-y-6">
              ...
              <div className="flex items-center justify-between">
                <div>
                  <h3 className={`text-sm font-black ${styles.cardText}`}>基于分析用途授权控制 (Purpose-Based Access Control - PBAC)</h3>
                  <p className={`text-[10px] ${styles.cardTextMuted}`}>只有定义合法、合规的数据分析「目的」(Purpose)，才能获得临时的密文解密密钥。任何违背目的的导出或越权行为将被严厉记录。</p>
                </div>
                <button
              ...
              <div className="grid grid-cols-1 md:grid-cols-3 gap-5">
                {purposes.map(p => (
                  <div key={p.id} className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-5 space-y-4`}>
                    <div className="flex items-start justify-between">
                      <span className={`px-2 py-0.5 rounded text-[8px] font-black ${
                       ...
                       <span className={`text-[9px] ${styles.cardTextMuted} font-mono`}>ID: {p.id}</span>
                    </div>

                       <div className="space-y-1">
                       <div className={`text-xs font-extrabold ${styles.cardText}`}>{p.name}</div>
                       <p className={`text-[10px] ${styles.cardTextMuted} leading-relaxed line-clamp-2`}>{p.description}</p>
                    </div>

                     <div className={`h-px ${styles.appBg}`} />

                     <div className={`space-y-2 text-[10px] ${styles.cardText}`}>
                      <div>
                         <span className={styles.cardTextMuted}>特许执行人员:</span>
                         <div className="flex flex-wrap gap-1 mt-1">
                           {p.authorizedUsers.map(u => (
                             <span key={u} className={`${styles.appBg} px-1.5 py-0.2 rounded font-mono ${styles.accentText}`}>
                              {u}
                            </span>
                          ))}
                        </div>
                      ...
                      <div>
                         <span className={styles.cardTextMuted}>锚定输入数据集:</span>
                         <div className="flex flex-wrap gap-1 mt-1">
                           {p.inputDatasets.map(ds => (
                             <span key={ds} className={`${styles.appBg} px-1.5 py-0.2 rounded font-mono ${styles.cardText}`}>
                              {ds}
                            </span>
                          ))}
                        </div>
                      ...
                      <div>
                         <span className={styles.cardTextMuted}>强制动态脱敏要求:</span>
                        <div className="flex flex-wrap gap-1 mt-1">
                      ...
                      <div className={`flex items-center justify-between text-[9px] border-t ${styles.cardBorder} pt-2 ${styles.cardTextMuted}`}>
                        <span>届期截至日期:</span>
                        <span className={`font-mono ${styles.cardTextMuted} font-semibold`}>{p.expiresAt}</span>
                      </div>
                    </div>
                    ...
                    <div className="flex items-center justify-end gap-1.5 pt-1">
                      {p.status === 'EXPIRED' ? (
                        <button
                          ...
                          className={`p-1 px-2.5 ${styles.badgeBg} ${styles.accentHover} text-[10px] font-bold ${styles.accentText} rounded cursor-pointer`}
                        >
                          重新激活
                        </button>
                      ) : (
          ...
          {activeTab === 'row_col' && (
            <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
              ...
              <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-4 space-y-4`}>
                <div className={`flex items-center justify-between border-b ${styles.cardBorder} pb-2`}>
                  <span className={`text-xs font-bold ${styles.cardText}`}>受动态策略约束的数据表</span>
                  <span className={`text-[9px] font-mono ${styles.cardTextMuted}`}>ROW/COL CONFIGS</span>
                </div>
                <div className="space-y-2">
                  {rowColPolicies.map(item => (
                    ...
                    >
                      <div className={`text-xs font-extrabold ${styles.cardText} truncate`}>{item.datasetName}</div>
                      <div className={`flex items-center gap-2 mt-1.5 text-[9px] ${styles.cardTextMuted}`}>
              ...
              <div className={`lg:col-span-2 ${styles.cardBg} border ${styles.cardBorder} rounded-xl p-5 space-y-6`}>
                {(() => {
                  ...
                  if (!currentPolicy) return <div className={`${styles.cardTextMuted} text-xs`}>请选择数据集</div>;

                  return (
                    <div className="space-y-6">
                      
                      <div className={`border-b ${styles.cardBorder} pb-4`}>
                        <h3 className={`text-sm font-black ${styles.cardText}`}>{currentPolicy.datasetName}</h3>
                        <p className={`text-[10px] ${styles.cardTextMuted}`}>在此处针对特定的列项设置动态遮蔽(SHA256哈希、全部抹除或部分脱敏)，或添加SQL风格的行级隔离条件。</p>
                      </div>

                      {/* Column Masking section */}
                      <div className="space-y-3">
                        <div className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`}>
                          <LucideIcon name="EyeOff" size={13} className={styles.accentText} />
                        ...
                        <div className={`border ${styles.cardBorder} rounded-lg overflow-hidden ${styles.appBg}`}>
                          <table className={`w-full text-left text-xs ${styles.cardText}`}>
                            ...
                            <tbody className={`divide-y ${styles.cardBorder} font-mono text-[11px]`}>
                              {currentPolicy.columnMasks.map(col => (
                                <tr key={col.column} className={`${styles.sidebarHoverBg}`}>
                                  <td className={`p-3 font-semibold ${styles.cardText}`}>{col.column}</td>
                                  <td className="p-3">
                                  ...
                                  <td className="p-3">
                                    <span className={`h-1.5 w-1.5 inline-block rounded-full mr-1.5 ${col.active && col.maskType !== 'NONE' ? 'bg-emerald-400 animate-pulse' : styles.cardTextMuted}`} />
                                  ...
                                  <td className="p-3 text-right">
                                    <button
                                      ...
                                      className={`p-1 px-2 text-[10px] font-bold rounded cursor-pointer ${
                                        ...
                                        col.active ? 'bg-emerald-950 text-emerald-400 hover:bg-emerald-900' : `${styles.sidebarActiveBg} ${styles.cardTextMuted} ${styles.sidebarHoverBg}`
                                      }`}
                                    >
                                      {col.active ? '激活生效' : '已关闭'}
                                    </button>
                      ...
                      <div className="space-y-3">
                        <div className={`text-xs font-bold ${styles.cardText} flex items-center gap-1.5`}>
                          <LucideIcon name="Filter" size={13} className={styles.accentText} />
                          <span>行级动态物理隔离策略 (Row-Level Security Policies)</span>
                        </div>
                        {currentPolicy.rowFilters.length === 0 ? (
                          <div className={`text-center p-6 ${styles.appBg} border ${styles.cardBorder} rounded-lg ${styles.cardTextMuted} text-xs`}>
                            当前数据集没有添加任何行过滤策略。
                          </div>
                        ) : (
                          <div className="space-y-2">
                            {currentPolicy.rowFilters.map((filter, idx) => (
                              <div key={idx} className={`${styles.appBg} border ${styles.cardBorder} p-3.5 rounded-lg flex items-center justify-between`}>
                                <div className="space-y-1">
                                  <div className={`text-[10px] ${styles.accentText} font-mono font-bold`}>SQL FILTER: {filter.filterSql}</div>
                                  <p className={`text-[10px] ${styles.cardTextMuted}`}>{filter.description}</p>
                                </div>
                                <button
                                  onClick={() => handleToggleRowFilter(currentPolicy.datasetId, idx)}
                                  className={`p-1.5 rounded text-[10px] font-black cursor-pointer ${
                                    filter.active ? 'bg-emerald-950 text-emerald-400 border border-emerald-900/40' : `${styles.sidebarActiveBg} ${styles.cardTextMuted}`
                                  }`}
                                >
                      ...
                      <div className={`${styles.appBg} border ${styles.cardBorder} rounded-lg p-4 space-y-3.5`}>
                        <div className={`flex items-center justify-between border-b ${styles.cardBorder} pb-2`}>
                          ...
                          <span className={`text-[8px] ${styles.sidebarActiveBg} px-1.5 py-0.5 rounded ${styles.cardTextMuted} font-mono`}>
                        ...
                        {currentPolicy.datasetId === 'ds_flight_schedules' ? (
                          <div className={`font-mono text-[10px] ${styles.cardText} space-y-2`}>
                            <div className={`grid grid-cols-3 gap-2 ${styles.cardBg} p-2 rounded ${styles.cardTextMuted} font-bold border-b ${styles.cardBorder}`}>
                              <span>航班号 (flight_num)</span>
                              <span>机长ID (captain_id)</span>
                              <span>延误时长 (delay_minutes)</span>
                            </div>
                            <div className={`grid grid-cols-3 gap-2 p-2 rounded ${styles.appBg} ${styles.sidebarHoverBg}`}>
                              <span>CA-1209</span>
                          ...
                          <div className={`text-center py-4 ${styles.cardTextMuted} text-xs`}>暂无可用预览行数据</div>
                        )}
                      </div>
                    </div>
                  );
            ...
            <div className="space-y-6">
              ...
              <div className={`flex items-center justify-between border-b ${styles.cardBorder} pb-4`}>
                <div className="space-y-1">
                  <h3 className={`text-sm font-black ${styles.cardText} flex items-center gap-2`}>
                    <LucideIcon name="Activity" size={15} className={styles.accentText} />
                ...
                <div className="flex items-center gap-2">
                  ...
                  <button
                    ...
                    className={`px-3 py-1.5 ${styles.accentBg} ${styles.accentHover} text-white rounded-lg text-xs font-bold transition-all flex items-center gap-1.5 cursor-pointer shadow-md`}
                  >
              ...
              <div className={`border ${styles.cardBorder} rounded-xl overflow-hidden ${styles.cardBg}`}>
                <table className={`w-full text-left text-xs ${styles.cardText}`}>
                  ...
                  <tbody className={`divide-y ${styles.cardBorder} font-mono text-[11px]`}>
                    {auditLogs.map(log => (
                      <tr key={log.id} className={`${styles.sidebarHoverBg} transition-colors`}>
                        <td className={`p-3 ${styles.accentText} font-bold`}>{log.timestamp}</td>
                        <td className={`p-3 ${styles.cardText}`}>{log.username}</td>
                        <td className={`p-3 ${styles.cardTextMuted}`}>{log.orgId.split('_')[1] || log.orgId}</td>
                        <td className={`p-3 ${styles.cardTextMuted} font-semibold`}>{log.action}</td>
                        <td className={`p-3 ${styles.cardTextMuted}`}>
                          <span className={`text-[10px] ${styles.accentText} font-bold ${styles.badgeBg} px-1 py-0.2 rounded ${styles.accentBorder} mr-1.5`}>
                            {log.resourceType}
                          </span>
                          {log.resourceId}
                        </td>
                        ...
                        <td className={`p-3 ${styles.cardTextMuted} max-w-sm truncate`} title={log.details}>
                          {log.details}
                        </td>
                      </tr>
                    ))}
          ...
          {activeTab === 'guide' && (
            <div className="space-y-6 animate-fadeIn">
              <div className={`flex items-center justify-between border-b ${styles.cardBorder} pb-4`}>
                <div className="space-y-1">
                  <h3 className={`text-sm font-black ${styles.cardText} flex items-center gap-2`}>
                    <LucideIcon name="BookOpen" size={15} className={styles.accentText} />
                    Foundry 零信任防御架构 - 安全中心系统管理员操作手册 (Security Operations Manual)
                  </h3>
                  <p className={`text-[10px] ${styles.cardTextMuted}`}>
                    本手册专为初级系统管理员设计，帮助其在极短时间内掌握组织初始化、项目授权、数据打标、行列脱敏与自适应审计的闭环防御能力。
                  </p>
                </div>
                <div className={`flex items-center gap-1.5 text-[10px] font-mono ${styles.accentText} ${styles.badgeBg} ${styles.accentBorder} px-3 py-1 rounded-lg`}>
                  <span className={`h-1.5 w-1.5 rounded-full ${styles.accentText} animate-ping inline-block mr-1`}></span>
                  <span>管理员: admin_guorong (超级特权模式)</span>
                </div>
              </div>

              {/* Sub-navigation inside guide tab */}
              <div className={`flex items-center gap-1.5 border-b ${styles.cardBorder} pb-3 overflow-x-auto scrollbar-none`}>
                <button
                  ...
                  className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
                    guideSection === 'arch' ? `${styles.accentBg} text-white shadow` : `${styles.sidebarBg} ${styles.cardTextMuted} hover:${styles.cardText} ${styles.sidebarHoverBg}`
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
                    guideSection === 'org' ? `${styles.accentBg} text-white shadow` : `${styles.sidebarBg} ${styles.cardTextMuted} hover:${styles.cardText} ${styles.sidebarHoverBg}`
                  }`}
                >
                ...
                <button
                  ...
                  className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
                    guideSection === 'dac' ? `${styles.accentBg} text-white shadow` : `${styles.sidebarBg} ${styles.cardTextMuted} hover:${styles.cardText} ${styles.sidebarHoverBg}`
                  }`}
                >
                  <LucideIcon name="FolderGit" size={12} />
                  <span>2. 项目自主授权 (DAC)</span>
                </button>
                <button
                  onClick={() => setGuideSection('mac')}
                  className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
                    guideSection === 'mac' ? `${styles.accentBg} text-white shadow` : `${styles.sidebarBg} ${styles.cardTextMuted} hover:${styles.cardText} ${styles.sidebarHoverBg}`
                  }`}
                >
                  <LucideIcon name="Tag" size={12} />
                  <span>3. 强制标记传播 (MAC)</span>
                </button>
                <button
                  onClick={() => setGuideSection('pbac')}
                  className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
                    guideSection === 'pbac' ? `${styles.accentBg} text-white shadow` : `${styles.sidebarBg} ${styles.cardTextMuted} hover:${styles.cardText} ${styles.sidebarHoverBg}`
                  }`}
                >
                  <LucideIcon name="EyeOff" size={12} />
                  <span>4. 目的授权与脱敏 (PBAC)</span>
                </button>
                <button
                  onClick={() => setGuideSection('audit')}
                  className={`px-3 py-1.5 text-[11px] rounded-lg font-bold transition-all flex items-center gap-1.5 cursor-pointer whitespace-nowrap ${
                    guideSection === 'audit' ? `${styles.accentBg} text-white shadow` : `${styles.sidebarBg} ${styles.cardTextMuted} hover:${styles.cardText} ${styles.sidebarHoverBg}`
                  }`}
                >
                ...
                {guideSection === 'arch' && (
                  <div className="space-y-4">
                    <div className={`flex items-center gap-2 border-b ${styles.cardBorder} pb-2`}>
                      <span className={`p-1.5 rounded-md ${styles.badgeBg} ${styles.accentText}`}>
                        <LucideIcon name="Shield" size={14} />
                      </span>
                      <h4 className={`text-xs font-black ${styles.cardText} uppercase tracking-wider`}>Foundry 四层深度防御物理/逻辑校验链路</h4>
                    </div>
                    
                    <p className={`text-xs ${styles.cardTextMuted} leading-relaxed`}>
                      Palantir Foundry 的安全底座不是靠单一防火墙或密保，而是依靠一套**零信任动态研判解密管道 (Zero-Trust Decryption Pipeline)**。
                    ...
                    <div className={`${styles.cardBg} border-l-4 ${styles.accentBorder} p-3.5 rounded-r-lg space-y-1 font-mono text-[10px] ${styles.cardTextMuted}`}>
                    ...
                    <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 text-xs ${styles.cardTextMuted}`}>
                      <div className="lg:col-span-2 space-y-3 leading-relaxed">
                        <p className="font-normal">
                          组织（Security Organization）是 Foundry 里的最高安全边界。一个用户只能属于一个主权组织，而数据可以打上组织物理隔离标签。
                        </p>
                      ...
                      <div className={`${styles.cardBg} border ${styles.cardBorder} p-4 rounded-xl space-y-3 text-[11px]`}>
                        <div className={`${styles.cardText} font-bold border-b ${styles.cardBorder} pb-1 flex items-center gap-1.5`}>
                          <LucideIcon name="CheckSquare" size={12} className="text-emerald-400" />
                          <span>初级管理员配置清单</span>
                        </div>
                  ...
                  <div className="space-y-4">
                    <div className={`flex items-center gap-2 border-b ${styles.cardBorder} pb-2`}>
                      <span className="p-1.5 rounded-md bg-blue-500/10 text-blue-400">
                        <LucideIcon name="FolderGit" size={14} />
                      </span>
                    ...
                    <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 text-xs ${styles.cardTextMuted}`}>
                      <div className="lg:col-span-2 space-y-3 leading-relaxed">
                        <p className="font-normal">
                          Foundry 采用类似于操作系统的“项目文件夹目录”进行粗粒度控制，我们称之为 **项目 ACL 授权 (DAC)**。
                      ...
                      <div className={`${styles.cardBg} border ${styles.cardBorder} p-4 rounded-xl space-y-3 text-[11px]`}>
                        <div className={`${styles.cardText} font-bold border-b ${styles.cardBorder} pb-1 flex items-center gap-1.5`}>
                          <LucideIcon name="Settings" size={12} className="text-blue-400" />
                ...
                {guideSection === 'mac' && (
                  <div className="space-y-4">
                    ...
                    <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 text-xs ${styles.cardTextMuted}`}>
                      <div className="lg:col-span-2 space-y-3 leading-relaxed">
                        <p className="font-normal">
                          普通的自主访问控制（DAC）很容易因分析人员意外将文件拷贝到另一个未受控文件夹而造成泄露。为了彻底封死此类漏洞，Foundry 推出了 **强制访问标记管理 (MAC - Security Markings)**。
                        </p>
                        <h5 className={`font-bold ${styles.cardText}`}>🏷️ 标记的最高行为特征 —— 级联向下游血缘传播 (Cascading Propagation)：</h5>
                        <p className="text-[11px] font-normal">
                          这是 Foundry 零信任安全机制中最著名的核心特点：
                      ...
                      <div className={`${styles.cardBg} border ${styles.cardBorder} p-4 rounded-xl space-y-3 text-[11px]`}>
                        <div className={`${styles.cardText} font-bold border-b ${styles.cardBorder} pb-1 flex items-center gap-1.5`}>
                          <LucideIcon name="Lock" size={12} className="text-pink-400" />
                    ...
                    <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 text-xs ${styles.cardTextMuted}`}>
                      <div className="lg:col-span-2 space-y-3 leading-relaxed">
                        ...
                        <ul className="list-disc list-inside space-y-1.5 text-[11px] pl-2 font-mono font-normal">
                          <li>
                            <strong className={`${styles.accentText}`}>列脱敏 (Column Masking - REDACT)</strong>: 完全隐藏整列。例将护照号全隐，流式解码引擎在明文层将其置换为 <code className={`${styles.cardTextMuted}`}>"** REDACTED **"</code>。
                          </li>
                          <li>
                            <strong className="text-amber-400">列脱敏 (Column Masking - PARTIAL)</strong>: 部分隐藏。例只保留电子邮箱邮箱后半段，前缀全掩，避免暴露客户的具体隐私（例如: <code className={`${styles.cardTextMuted}`}>"t***@gmail.com"</code>）。
                          </li>
                          <li>
                            <strong className="text-emerald-400">行过滤 (Row Filter Predicate)</strong>: 注入谓词过滤 SQL，限制物理可见行。例如在行中注入 <code className={`${styles.cardTextMuted}`}>"org_id = 'Org_EU_Ops'"</code>，当前用户如果访问此表，所有非欧盟组织范围内的记录将直接从底层物理引擎上蒸发，无法被探测！
                          </li>
                        </ul>
                      </div>
                      ...
                      <div className={`${styles.cardBg} border ${styles.cardBorder} p-4 rounded-xl space-y-3 text-[11px]`}>
                        <div className={`${styles.cardText} font-bold border-b ${styles.cardBorder} pb-1 flex items-center gap-1.5`}>
                          <LucideIcon name="Settings" size={12} className="text-emerald-400" />
                ...
                {guideSection === 'audit' && (
                  <div className="space-y-4">
                    ...
                    <div className={`grid grid-cols-1 lg:grid-cols-3 gap-6 text-xs ${styles.cardTextMuted}`}>
                      ...
                      <div className={`${styles.cardBg} border ${styles.cardBorder} p-4 rounded-xl space-y-3 text-[11px]`}>
                        <div className={`${styles.cardText} font-bold border-b ${styles.cardBorder} pb-1 flex items-center gap-1.5`}>
                          <LucideIcon name="ShieldAlert" size={12} className="text-amber-400" />
                          <span>态势感知与阻断核验</span>
                        </div>
              ...
              <div className={`p-4 ${styles.cardBg} border ${styles.cardBorder} rounded-xl flex items-center justify-between text-xs`}>
                <div className="flex items-center gap-3">
                  <span className={`p-2 ${styles.badgeBg} ${styles.accentText} rounded-lg ${styles.accentBorder}`}>
      ...
      {showAddOrgModal && (
        <div className="fixed inset-0 bg-black/60 backdrop-blur-xs flex items-center justify-center z-50 p-4">
          <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-xl p-6 max-w-md w-full space-y-4`}>
            ...
            <form onSubmit={handleAddOrg} className="space-y-3 text-xs">
              <div>
                ...
                <input
                  ...
                  className={`w-full ${styles.inputBg} border ${styles.inputBorder} rounded p-2 ${styles.inputText} outline-none`}
                />
              </div>
              <div>
                ...
                <input
                  ...
                  className={`w-full ${styles.inputBg} border ${styles.inputBorder} rounded p-2 ${styles.inputText} font-mono outline-none`}
                />
              </div>
              <div>
              ...
              <div className="flex items-center gap-2 pt-2">
                <input
                  ...
                  className={`rounded ${styles.sidebarBg} ${styles.cardBorder}`}
                />
                <label htmlFor="isolation_chk" className={`${styles.cardTextMuted} font-semibold cursor-pointer select-none`}>
                  强制启用物理域高耸严密隔离
                </label>
              </div>
              <div className={`flex justify-end gap-2 pt-4 border-t ${styles.cardBorder}`}>
                <button
                  type="button"
                  onClick={() => setShowAddOrgModal(false)}
                  className={`px-3 py-1.5 ${styles.cardBg} ${styles.cardTextMuted} rounded-lg ${styles.sidebarHoverBg} cursor-pointer`}
                >
                  取消
                </button>
                <button
            ...
            <form onSubmit={handleAddMarking} className="space-y-3 text-xs">
              ...
              <div>
                ...
                <textarea
                  ...
                  className={`w-full ${styles.inputBg} border ${styles.inputBorder} rounded p-2 ${styles.inputText} outline-none h-16 resize-none`}
                />
              </div>
              <div className={`flex justify-end gap-2 pt-4 border-t ${styles.cardBorder}`}>
                <button
      ...
      {showDiagnosticModal && (
        <div className="fixed inset-0 bg-black/80 backdrop-blur-md flex items-center justify-center z-50 p-4">
          <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-2xl p-6 max-w-4xl w-full max-h-[85vh] overflow-hidden flex flex-col space-y-4 shadow-2xl relative`}>
            <div className="absolute top-4 right-4 flex items-center gap-2">
              ...
              <button onClick={() => setShowDiagnosticModal(false)} className={`${styles.cardTextMuted} hover:${styles.cardText} cursor-pointer p-1 ${styles.cardBg} ${styles.sidebarHoverBg} rounded`}>
                <LucideIcon name="X" size={16} />
              </button>
            </div>
            ...
            <div className={`border-b ${styles.cardBorder} pb-3 flex items-center gap-2.5`}>
              <span className="p-2 bg-emerald-950/50 rounded-lg text-emerald-400 border border-emerald-800/30">
                <LucideIcon name="ShieldCheck" size={18} className="animate-pulse" />
              </span>
              <div>
            ...
            <div className={`flex-1 overflow-y-auto space-y-3 pr-2 scrollbar-thin scrollbar-thumb-slate-800 ${styles.cardTextMuted} text-xs font-sans leading-relaxed`}>
              <div className={`p-4 ${styles.appBg} border ${styles.cardBorder} rounded-xl space-y-3`}>
                {diagnosticReport.split('\n').map((line, idx) => {
                  ...
                  if (line.startsWith('### ')) {
                    return (
                      <h6 key={idx} className={`${styles.accentText} font-black text-xs pt-2 flex items-center gap-1.5`}>
                        <LucideIcon name="ShieldAlert" size={12} className={`${styles.accentText}`} />
                        {line.replace('### ', '')}
                      </h6>
                    );
                  }
                  ...
                  if (/^\d+\s*\./.test(line.trim())) {
                    return (
                      <div key={idx} className={`ml-4 pl-3 border-l ${styles.accentBorder} py-1 ${styles.appBg} rounded-r text-[11px] font-mono flex flex-col gap-1`}>
                  ...
                  if (line.trim().startsWith('*判定依据*') || line.trim().startsWith('- *判定依据*')) {
                    return (
                      <div key={idx} className={`ml-10 text-[10px] ${styles.cardTextMuted} italic ${styles.appBg} p-1.5 rounded border ${styles.cardBorder}`}>
                        {line}
                      </div>
                    );
                  }
            ...
            <div className={`border-t ${styles.cardBorder} pt-3 flex justify-between items-center text-[10px] ${styles.cardTextMuted} font-mono`}>
              ...
              <button
                ...
                className={`px-4 py-1.5 ${styles.cardBg} ${styles.cardTextMuted} font-black rounded-lg ${styles.sidebarHoverBg} cursor-pointer border ${styles.cardBorder} transition-all text-xs`}
              >
                关闭诊断书
              </button>
            </div>
