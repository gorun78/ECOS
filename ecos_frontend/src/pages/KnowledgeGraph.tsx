...
export default function KnowledgeGraphPage() {
  ...
  return (
    <div className={`flex-1 ${styles.appBg} flex flex-col h-full font-sans overflow-hidden`}>
      {/* Header */}
      <div className={`${styles.cardBg} border-b ${styles.cardBorder} p-4 shrink-0`}>
        <div className="flex items-center justify-between gap-4">
          <div>
            ...
            <p className={`text-[11px] ${styles.cardTextMuted} mt-0.5`}>
              {kgNodes.length} {locale === "zh" ? "实体" : "entities"} · {kgEdges.length} {locale === "zh" ? "关系" : "relations"}
            </p>
          </div>
          <div className="flex items-center gap-2">
            <div className={`flex items-center gap-1.5 ${styles.appBg} border ${styles.cardBorder} rounded-lg px-2.5 py-1`}>
              <Search className={`w-3 h-3 ${styles.cardTextMuted}`} />
              <input type="text"
                className={`bg-transparent outline-none text-[11px] ${styles.cardText} placeholder:opacity-50 w-36`}
            ...
            <button onClick={() => { setSearchQuery(""); loadGraph(); }}
              className={`p-1.5 rounded ${styles.sidebarHoverBg} ${styles.cardTextMuted}`}>
        ...
        {/* ── Path Highlight Row ── */}
        <div className={`flex items-center gap-2 mt-3 pt-3 border-t ${styles.divider}`}>
          <Route className={`w-3.5 h-3.5 ${styles.cardTextMuted}`} />
          <span className={`text-[10px] ${styles.cardTextMuted} font-semibold uppercase tracking-wide`}>
            {locale === "zh" ? "路径查询" : "Path Find"}
          </span>
          <select
            value={pathSource}
            onChange={e => { setPathSource(e.target.value); clearPathHighlight(); }}
            className={`text-[11px] border ${styles.cardBorder} rounded-md px-2 py-1 ${styles.cardBg} ${styles.cardText} outline-none focus:border-indigo-400`}
          >
          ...
          <button
            ...
            className="px-2.5 py-1 bg-red-500 hover:bg-red-600 disabled:bg-[var(--card,#475569)] text-white rounded-md text-[10px] font-semibold transition flex items-center gap-1"
          >
          ...
          {pathNodeIds.size > 0 && (
            <button onClick={clearPathHighlight} className={`text-[10px] ${styles.cardTextMuted} hover:opacity-60 underline`}>
              {locale === "zh" ? "清除" : "Clear"}
            </button>
          )}
        </div>
      ...
      {!loading && graphNodes.length > 0 && (
        <div className={`px-4 py-2 ${styles.cardBg} border-b ${styles.cardBorder} shrink-0`}>
          <div className="flex items-center gap-4 flex-wrap">
            <div className={`flex items-center gap-2 px-3 py-1.5 ${styles.appBg} rounded-lg`}>
              <BarChart3 className="w-3.5 h-3.5 text-indigo-500" />
              <div>
                <span className={`text-[9px] ${styles.cardTextMuted} uppercase tracking-wide block`}>
                  {locale === "zh" ? "总节点" : "Nodes"}
                </span>
                <span className={`text-sm font-bold ${styles.cardText}`}>{graphStats.nodeCount}</span>
              </div>
            </div>
            <div className={`flex items-center gap-2 px-3 py-1.5 ${styles.appBg} rounded-lg`}>
              ...
              <div>
                ...
                <span className={`text-sm font-bold ${styles.cardText}`}>{graphStats.edgeCount}</span>
              </div>
            </div>
            <div className={`flex items-center gap-2 px-3 py-1.5 ${styles.appBg} rounded-lg`}>
              <span className={`text-[9px] ${styles.cardTextMuted} uppercase tracking-wide`}>
            ...
            <div className={`flex items-center gap-2 px-3 py-1.5 ${styles.appBg} rounded-lg`}>
              ...
              <span className={`text-sm font-bold ${styles.cardText}`}>
        ...
        <div className="flex-1 min-w-0 flex">
          {loading ? (
            <div className="flex items-center justify-center h-full">
              <RefreshCw className={`w-10 h-10 ${styles.cardTextMuted} opacity-50 animate-spin`} />
            </div>
          ) : graphNodes.length === 0 ? (
            <div className="flex items-center justify-center h-full">
              <div className="text-center space-y-3">
                <Network className={`w-10 h-10 ${styles.cardTextMuted} opacity-40 mx-auto`} />
                <p className={`text-sm ${styles.cardTextMuted}`}>
          ...
          {expandLoading && (
            <div className={`absolute bottom-4 right-4 z-10 ${styles.cardBg} border ${styles.cardBorder} rounded-lg px-3 py-1.5 shadow flex items-center gap-2`}>
        ...
        {selectedNode && (
          <div className={`w-full lg:w-[300px] ${styles.cardBg} border-l ${styles.cardBorder} p-5 overflow-y-auto shrink-0 shadow-lg`}>
            ...
            <div className="space-y-3 text-xs">
              <div>
                <span className={`text-[10px] ${styles.cardTextMuted} uppercase tracking-wider block`}>ID</span>
                <span className={`font-mono ${styles.cardText} text-[11px]`}>{selectedNode.id}</span>
              </div>
              <div>
                <span className={`text-[10px] ${styles.cardTextMuted} uppercase tracking-wider block`}>{locale === "zh" ? "名称" : "Name"}</span>
                <span className={`font-semibold ${styles.cardText}`}>{selectedNode.label || selectedNode.name || selectedNode.id}</span>
              </div>
              <div>
                <span className={`text-[10px] ${styles.cardTextMuted} uppercase tracking-wider block`}>{locale === "zh" ? "类型" : "Type"}</span>
              ...
              {selectedNode.description && (
                <div>
                  <span className={`text-[10px] ${styles.cardTextMuted} uppercase tracking-wider block`}>{locale === "zh" ? "描述" : "Description"}</span>
                  <p className={`${styles.cardText} leading-relaxed`}>{selectedNode.description}</p>
                </div>
              )}
              {selectedNode.createdAt && (
                <div>
                  <span className={`text-[10px] ${styles.cardTextMuted} uppercase tracking-wider block`}>{locale === "zh" ? "创建时间" : "Created"}</span>
                  <span className={styles.cardTextMuted}>{selectedNode.createdAt}</span>
                </div>
              )}
              ...
              {(selectedNode.properties || (selectedNode.propertiesJson && (() => { try { return JSON.parse(selectedNode.propertiesJson); } catch { return null; } })())) && (
                <div className={`pt-3 border-t ${styles.divider}`}>
                  <span className={`text-[10px] ${styles.cardTextMuted} uppercase tracking-wider block mb-2`}>
                  ...
                  <div className="space-y-1.5">
                    {Object.entries(
                      ...
                      <div key={k} className={`flex justify-between ${styles.appBg} rounded px-2 py-1`}>
                        <span className={`font-mono text-[10px] ${styles.cardTextMuted}`}>{k}</span>
                        <span className={`text-[10px] ${styles.cardText} truncate ml-2 max-w-[140px]`}>{String(v)}</span>
                      </div>
                    ))}
                  </div>
                </div>
                    ...
                    {relatedNodes.map(rn => {
                      ...
                      return (
                        <button key={rn.id}
                          ...
                          {edge?.label && <span className={`text-[9px] ${styles.cardTextMuted} ml-auto`}>{edge.label}</span>}
                        </button>
                      );
                    })}
                  </div>
