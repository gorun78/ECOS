...
<div className="space-y-4">
  ...
  <div className="bg-[var(--card,#020617)] border border-[var(--card,#1E293B)] p-4 rounded-xl">
    ...
    <div className="grid grid-cols-1 md:grid-cols-6 gap-3 items-center relative text-center">
      ...
      <div className="bg-[var(--card,#0F172A)]/90 border border-[var(--card,#1E293B)] p-3 rounded-lg relative flex flex-col items-center">
        <div className="p-2 rounded-full bg-blue-950 text-blue-400 mb-1.5">
          <LucideIcon name="Database" size={14} />
        </div>
        <span className="text-[10px] font-bold text-[var(--card,#CBD5E1)]">1. 数据资产 (Data)</span>
        <div className="mt-2 space-y-1 w-full">
          {activeScenario.bindings.datasets.map(d => (
            <span key={d} className="block text-[8px] bg-[var(--card,#0B0F19)] px-1 py-0.5 rounded border border-[var(--card,#1E293B)] font-mono text-[var(--card,#94A3B8)] truncate">{d}</span>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[var(--card,#0F172A)]/90 border border-[var(--card,#1E293B)] p-3 rounded-lg relative flex flex-col items-center">
        ...
        <span className="text-[10px] font-bold text-[var(--card,#CBD5E1)]">2. 本体映射 (Ontology)</span>
        <div className="mt-2 space-y-1 w-full">
          {activeScenario.bindings.objectTypes.map(o => (
            <span key={o} className="block text-[8px] bg-[var(--card,#0B0F19)] px-1 py-0.5 rounded border border-[var(--card,#1E293B)] font-mono text-[var(--card,#94A3B8)] truncate">{o}</span>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[var(--card,#0F172A)]/90 border border-[var(--card,#1E293B)] p-3 rounded-lg relative flex flex-col items-center">
        ...
        <span className="text-[10px] font-bold text-[var(--card,#CBD5E1)]">3. 知识背景 (RAG)</span>
        <div className="mt-2 space-y-1 w-full">
          {activeScenario.bindings.knowledgeBases.map(k => (
            <span key={k} className="block text-[8px] bg-[var(--card,#0B0F19)] px-1 py-0.5 rounded border border-[var(--card,#1E293B)] font-sans text-[var(--card,#94A3B8)] truncate">{k}</span>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[var(--card,#0F172A)]/90 border border-[var(--card,#1E293B)] p-3 rounded-lg relative flex flex-col items-center">
        ...
        <span className="text-[10px] font-bold text-[var(--card,#CBD5E1)]">4. AI 决策 (Agents)</span>
        <div className="mt-2 space-y-1 w-full">
          {activeScenario.bindings.aiAgents.map(a => (
            <span key={a} className="block text-[8px] bg-[var(--card,#0B0F19)] px-1 py-0.5 rounded border border-[var(--card,#1E293B)] font-sans text-[var(--card,#94A3B8)] truncate">{a}</span>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[var(--card,#0F172A)]/90 border border-[var(--card,#1E293B)] p-3 rounded-lg relative flex flex-col items-center">
        ...
        <span className="text-[10px] font-bold text-[var(--card,#CBD5E1)]">5. 安全围栏 (Security)</span>
        <div className="mt-2 space-y-1 w-full">
          {activeScenario.bindings.securityPolicies.map(s => (
            <span key={s} className="block text-[8px] bg-[var(--card,#0B0F19)] px-1 py-0.5 rounded border border-[var(--card,#1E293B)] font-mono text-[var(--card,#94A3B8)] truncate">{s}</span>
          ))}
        </div>
      </div>
      ...
      <div className="bg-[var(--card,#0F172A)]/90 border border-[var(--card,#1E293B)] p-3 rounded-lg relative flex flex-col items-center">
        ...
        <span className="text-[10px] font-bold text-[var(--card,#CBD5E1)]">6. 构建面板 (App UI)</span>
        <div className="mt-2 space-y-1 w-full">
          {activeScenario.bindings.interfaces.map(i => (
            <span key={i} className="block text-[8px] bg-[var(--card,#0B0F19)] px-1 py-0.5 rounded border border-[var(--card,#1E293B)] font-sans text-[var(--card,#94A3B8)] truncate">{i}</span>
          ))}
        </div>
      </div>
