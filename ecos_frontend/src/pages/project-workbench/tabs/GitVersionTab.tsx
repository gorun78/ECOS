...
export default function GitVersionTab(props: GitVersionTabProps) {
  ...
  return (
    <div className="space-y-4">
      {/* Git Status Bar */}
      <div className="bg-[var(--card,#0F172A)] border border-[var(--card,#1E293B)] rounded-xl p-4 flex items-center gap-4">
        <div className="flex items-center gap-2 p-2 bg-[var(--card,#020617)] rounded border border-[var(--card,#1E293B)]">
          <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse" />
          <span className="text-xs font-mono font-bold text-[var(--card,#E2E8F0)]">
      ...
      {/* Commit History */}
      <div className="bg-[var(--card,#020617)] border border-[var(--card,#1E293B)] rounded-xl overflow-hidden">
        <div className="px-4 py-3 bg-[var(--card,#020617)]/80 border-b border-[var(--card,#1E293B)] flex items-center justify-between">
        ...
        {currentCommits.length === 0 ? (
            ...
            {[...currentCommits].reverse().map(commit => (
              <div
                ...
                className={`p-3 cursor-pointer transition-all ${
                  selectedCommitId === commit.id ? 'bg-indigo-950/40 border-l-2 border-indigo-500' : 'hover:bg-[var(--card,#0F172A)]/60'
                }`}
              >
                <div className="flex items-center justify-between">
                  <div className="flex items-center gap-2">
                    <span className="text-[11px] font-mono text-amber-400 font-bold">{commit.hash}</span>
                    <span className="text-[11px] font-sans text-[var(--card,#E2E8F0)] truncate max-w-md">{commit.message}</span>
                  </div>
                  <div className="flex items-center gap-2 shrink-0">
                    <span className="text-[10px] text-[var(--card,#64748B)] font-mono">{commit.date}</span>
                    <span className="text-[10px] text-[var(--card,#94A3B8)]">by {commit.author}</span>
                  </div>
                </div>
          ...
          <pre className="p-4 text-[11px] font-mono text-[var(--card,#CBD5E1)] max-h-72 overflow-y-auto whitespace-pre-wrap">
            ...
            <button
              ...
              className={`px-2.5 py-1 rounded text-[10px] font-mono font-bold cursor-pointer transition-all ${
                currentBranch === branch
                  ...
                  : 'bg-[var(--card,#1E293B)] text-[var(--card,#94A3B8)] border border-[var(--card,#334155)] hover:border-[var(--card,#475569)]'
              }`}
            >
              {branch}
            </button>
      ...
      {gitTerminalLogs.length > 0 && (
        <div className="bg-black border border-[var(--card,#1E293B)] rounded-xl overflow-hidden">
          <div className="px-3 py-2 bg-[var(--card,#020617)] border-b border-[var(--card,#1E293B)] flex items-center gap-2">
            <span className="w-2.5 h-2.5 rounded-full bg-red-500" />
            <span className="w-2.5 h-2.5 rounded-full bg-amber-500" />
            <span className="w-2.5 h-2.5 rounded-full bg-emerald-500" />
            <span className="text-[10px] text-[var(--card,#64748B)] ml-2 font-mono">ECOS Git Terminal</span>
          </div>
