    ...
    <div className="space-y-4">
      ...
      {tpl && TIcon && (
        <div className={`${styles.cardBg} border ${styles.cardBorder} rounded-lg p-3 flex items-center gap-2`}>
          ...
          <span className={`text-[10px] ${styles.cardTextMuted} font-mono`}>{tpl.type}</span>
        </div>
      )}
      <div>
          ...
          <div className={`border ${styles.cardBorder} rounded-lg max-h-32 overflow-y-auto p-2`}>
            {cols.map(c => (
              <label key={c.name} className={`flex items-center gap-2 py-1 cursor-pointer hover:${styles.cardBg} rounded px-1`}>
                ...
                <span className={`text-[11px] ${styles.cardTextMuted}`}>{c.name}</span>
                <span className={`text-[9px] ${styles.cardTextMuted} ml-auto`}>{c.type}</span>
              </label>
            ))}
          </div>
        </div>
          ...
          <div className="space-y-2">
            {Object.entries(tpl.defaultParams).map(([k, v]) => (
              <div key={k} className="flex items-center gap-2">
                <span className={`text-[10px] ${styles.muted} w-20 shrink-0`}>{k}</span>
                <input className={`flex-1 border ${styles.inputBorder} ${styles.inputBg} rounded px-2 py-1 text-[10px] outline-none focus:${styles.infoBorder}`}
