 ...
 * 分层结构：按段内子分区（subgroups）分组渲染，每分区前一行小节标题；无子分区时平铺。
...
import type { ConfigGroup, ConfigItem, ConfigValues, DefaultValues } from './DataEngineConfigPanelTypes';
...
interface Props {
  ...
  /** 当前段；loading/错误态下可能为 undefined（此时仅渲染状态占位） */
...
const DataEngineConfigPanelForm: React.FC<Props> = ({
  ...
  const hasSections = subgroups.length > 0;
  // 未命中任何子分区的项（后端新增子分区但前端未同步等）→ 末尾兜底，不静默丢弃
  const ungroupedItems = hasSections
    ? items.filter(item => !subgroups.some(sg => sg.id === item.subgroup))
    : [];
  ...
  /** 渲染子分区小节（标题 + 该分区配置项）；分区为空时不渲染 */
  const renderSection = (id: string, labelKey: string, sectionItems: ConfigItem[]) => {
    if (sectionItems.length === 0) {
      return null;
    }
    return (
      <div key={id} className="space-y-4">
        <div className={`font-mono text-[10px] tracking-wider uppercase ${styles.cardTextMuted} pb-1 border-b ${styles.cardBorder}`}>
          {t(labelKey)}
        </div>
  ...
  return (
    <div className="flex-1 flex flex-col min-h-0 overflow-y-auto">
      ...
      {currentGroup && (
        <div className={`px-5 py-3 border-b ${styles.cardBorder} ${styles.cardBg} sticky top-0 z-10`}>
          ...
          <p className={`text-[10px] ${styles.cardTextMuted} mt-0.5 ml-7`}>
            {t('dw.cfg.form.itemCount', { count: items.length })}
          </p>
        </div>
      )}
      ...
      <div className="px-5 py-4 space-y-4">
        {items.length === 0 ? (
          <div className={`text-xs ${styles.cardTextMuted} py-6 text-center`}>
            {t('dw.cfg.form.emptyGroup')}
          </div>
        ) : hasSections ? (
          <>
            {subgroups.map(sg =>
              renderSection(sg.id, sg.labelKey, items.filter(item => item.subgroup === sg.id)))}
            {renderSection('general', 'dw.cfg.subgroup.general', ungroupedItems)}
          </>
        ) : (
