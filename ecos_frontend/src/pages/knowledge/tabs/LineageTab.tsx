...
export default function LineageTab() {
  ...
  const handleParseLineage = async () => {
    ...
    try {
      let parsedObj;
      try { parsedObj = JSON.parse(rawPayload); } catch { alert(t("knowledge.lineagetab.json格式错误")); setIsParsing(false); return; }
    ...
    <div className="space-y-6">
      <div className="flex items-center justify-between border-b border-slate-200 pb-3">
        <div className="space-y-1">
          <h2 className="text-sm font-black text-slate-800">{t("knowledge.lineagetab.元数据血缘解析器与下游影响分析")}</h2>
          <p className="text-xs text-slate-500">{t("knowledge.lineagetab.支持_openlineage_及_apache_atlas_")}</p>
        </div>
      </div>
      ...
      <div className="grid grid-cols-1 xl:grid-cols-2 gap-6">
        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs flex flex-col space-y-4">
          <div className="flex items-center justify-between border-b border-slate-100 pb-2.5">
            <h3 className="font-extrabold text-slate-800 text-xs flex items-center gap-1.5"><Code size={13} className="text-indigo-600" /><span>{t("knowledge.lineagetab.血缘解析器")}</span></h3>
          ...
          <button onClick={handleParseLineage} disabled={isParsing || !rawPayload.trim()} className="w-full py-2 bg-indigo-600 hover:bg-indigo-700 text-white font-extrabold rounded-lg shadow-sm transition-all flex items-center justify-center gap-1.5 cursor-pointer text-xs disabled:opacity-60">
            {isParsing ? <><span className="w-3 h-3 border-2 border-white border-t-transparent rounded-full animate-spin" /><span>{t("knowledge.lineagetab.解析中")}</span></> : <><FileInput size={12} /><span>{t("knowledge.lineagetab.解析并合并血缘_dag")}</span></>}
          </button>
        </div>

        <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs flex flex-col space-y-4">
          <h3 className="font-extrabold text-slate-800 text-xs border-b border-slate-100 pb-2.5 flex items-center gap-1.5"><Network size={13} className="text-emerald-600" /><span>{t("knowledge.lineagetab.多维级联血缘拓扑_dag")} ({lineageNodes.length} nodes)</span></h3>
          <div className="flex-1 bg-slate-50 border border-slate-200/60 rounded-xl p-4 overflow-y-auto max-h-[300px] space-y-3">
            {['physical_table', 'etl_job', 'olap_table', 'ontology_object', 'dashboard'].map((type, i) => {
              ...
              const labels = [t("knowledge.lineagetab.物理数据源"), t("knowledge.lineagetab.etl管道"), t("knowledge.lineagetab.olap宽表"), t("knowledge.lineagetab.本体对象"), t("knowledge.lineagetab.下游报表")];
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs space-y-4">
        <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-100 pb-3 gap-3">
          <h3 className="font-extrabold text-slate-800 text-xs flex items-center gap-1.5"><ShieldCheck size={13} className="text-rose-600" /><span>{t("knowledge.lineagetab.级联影响分析")}</span></h3>
          <div className="flex items-center gap-2">
            <span className="text-[10px] font-bold text-slate-500">{t("knowledge.lineagetab.选择分析对象")}</span>
        ...
        {isAnalyzing ? (
          <div className="py-12 flex flex-col items-center justify-center space-y-2"><span className="w-6 h-6 border-2 border-indigo-600 border-t-transparent rounded-full animate-spin" /><span className="text-slate-500 font-bold text-[10px]">{t("knowledge.lineagetab.推导中")}</span></div>
        ) : impactResult ? (
          <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
            <div className="bg-slate-50 border border-slate-200 rounded-xl p-4 flex flex-col items-center justify-center text-center space-y-3 relative overflow-hidden">
              ...
              <span className="text-[9px] font-extrabold text-slate-400 uppercase tracking-widest font-mono">{t("knowledge.lineagetab.风险评分")}</span>
            ...
            <div className="lg:col-span-2 bg-white border border-slate-200 rounded-xl p-4 space-y-3">
              <h4 className="font-extrabold text-slate-700 text-[10px] uppercase tracking-wider border-b border-slate-100 pb-2">{t("knowledge.lineagetab.影响组件")} ({impactResult.impactedNodes?.length || 0})</h4>
              ...
              ) : <div className="py-6 text-center text-slate-400 text-[10px]"><ShieldAlert size={18} className="mx-auto text-slate-300 mb-1" />{t("knowledge.lineagetab.无下游依赖")}</div>}
            </div>
          </div>
        ) : <div className="py-6 text-center text-slate-400 text-[10px]">{t("knowledge.lineagetab.请选择分析对象")}</div>}
      </div>
    </div>
  );
}
