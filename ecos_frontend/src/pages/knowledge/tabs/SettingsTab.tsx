...
export default function SettingsTab() {
  ...
  const handleSave = async () => {
    ...
    try {
      ...
      setToast({ type: 'success', msg: t("knowledge.settingstab.配置已保存") });
    } catch {
    ...
    <div className="space-y-6 max-w-3xl">
      <div className="flex items-center justify-between border-b border-slate-200 pb-3">
        <div className="space-y-1">
          <h2 className="text-sm font-black text-slate-800 flex items-center gap-2">
            ...
            {t("knowledge.settingstab.工作台配置_knowledge_settings")}
          </h2>
          <p className="text-xs text-slate-500">
            {t("knowledge.settingstab.配置知识工作台的全局参数_包括向量模型_切块策略_neo4j")}
          </p>
        </div>
      ...
      <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs space-y-5">
        <h3 className="font-extrabold text-slate-800 text-xs border-b border-slate-100 pb-2">
          {t("knowledge.settingstab.向量索引配置")}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-1">
            <label className="block text-slate-600 font-bold text-[10px] uppercase">{t("knowledge.settingstab.默认向量模型")}</label>
          ...
          <div className="space-y-1">
            <label className="block text-slate-600 font-bold text-[10px] uppercase">{t("knowledge.settingstab.默认切块大小")}</label>
          ...
          <div className="space-y-1">
            <label className="block text-slate-600 font-bold text-[10px] uppercase">{t("knowledge.settingstab.默认重叠度")}</label>
        ...
        <h3 className="font-extrabold text-slate-800 text-xs border-b border-slate-100 pb-2 pt-2">
          {t("knowledge.settingstab.检索与同步配置")}
        </h3>
        <div className="grid grid-cols-1 md:grid-cols-3 gap-4">
          <div className="space-y-1">
            <label className="block text-slate-600 font-bold text-[10px] uppercase">{t("knowledge.settingstab.最大检索结果数")}</label>
          ...
          <div className="flex items-center gap-3 pt-4">
            <label className="text-slate-600 font-bold text-[10px] uppercase">{t("knowledge.settingstab.启用neo4j")}</label>
          ...
          <div className="flex items-center gap-3 pt-4">
            <label className="text-slate-600 font-bold text-[10px] uppercase">{t("knowledge.settingstab.自动同步")}</label>
