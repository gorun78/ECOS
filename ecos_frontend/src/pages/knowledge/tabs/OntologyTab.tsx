    ...
    <div className="space-y-6">
      <div className="flex flex-col md:flex-row md:items-center justify-between border-b border-slate-200 pb-4 gap-4">
        <div className="space-y-1">
          <h2 className="text-sm font-black text-slate-800 flex items-center gap-2"><Workflow size={16} className="text-blue-600 animate-pulse" />{t("knowledge.ontologytab.语义本体与物理宽表对齐管理器")}</h2>
          <p className="text-xs text-slate-500 font-sans">{t("knowledge.ontologytab.建立强类型对齐契约_将逻辑本体字段与物理大宽表列名进行多对多")}</p>
        </div>
        ...
        <div className="lg:col-span-3 space-y-4">
          <div className="bg-white border border-slate-200 rounded-xl p-4 shadow-xs space-y-3">
            <div className="flex items-center justify-between border-b border-slate-100 pb-2">
              <h3 className="font-extrabold text-slate-800 text-xs flex items-center gap-1.5"><Layers size={12} className="text-slate-500" /><span>{t("knowledge.ontologytab.语义本体实体")}</span></h3>
            ...
            <div className="bg-white border border-slate-200 rounded-xl p-5 shadow-xs space-y-5">
              ...
              <div className="space-y-3">
                <div className="flex items-center justify-between">
                  <span className="text-[10px] font-extrabold text-slate-400 uppercase tracking-wider font-mono">{t("knowledge.ontologytab.对齐映射清单")}</span>
                  ...
                  <table className="w-full text-left border-collapse">
                    ...
                    <tbody className="divide-y divide-slate-100 text-[11px]">
                      {(!editingOntology.mappings || editingOntology.mappings.length === 0) ? (
                        <tr><td colSpan={6} className="p-8 text-center text-slate-400 font-sans">{t("knowledge.ontologytab.尚未配置映射")}</td></tr>
              ...
              <div className="flex items-center justify-between pt-4 border-t border-slate-100">
                <div className="text-[10px] text-slate-400">* {t("knowledge.ontologytab.保存后实时更新_rag_上下文数据库")}</div>
                <button onClick={() => handleSaveMappings(ontologyMappings)} className="px-5 py-2 bg-slate-900 hover:bg-slate-800 text-white font-extrabold rounded-lg shadow-sm flex items-center gap-1.5 cursor-pointer text-xs transition-colors"><Save size={12} /><span>{t("knowledge.ontologytab.保存对齐契约")}</span></button>
              </div>
            </div>
          ) : (
            <div className="bg-white border border-slate-200 rounded-xl p-12 text-center text-slate-400 space-y-2"><Workflow size={24} className="mx-auto text-slate-300 animate-pulse" /><p className="font-bold text-xs">{t("knowledge.ontologytab.请选择或创建本体对象")}</p></div>
          )}
        </div>
      </div>
        ...
        <div className="fixed inset-0 z-50 flex items-center justify-center p-4 bg-slate-900/60 backdrop-blur-xs animate-fade-in">
          <div className="bg-white rounded-2xl max-w-2xl w-full border border-slate-200 shadow-xl overflow-hidden flex flex-col max-h-[85vh]">
            ...
            <div className="p-4 bg-slate-50 border-t border-slate-100 flex items-center justify-end gap-2 shrink-0">
              ...
              <button onClick={() => setShowExportModal(false)} className="px-4 py-1.5 bg-slate-200 hover:bg-slate-300 text-slate-700 font-bold rounded-lg text-xs cursor-pointer transition-all">{t("knowledge.ontologytab.关闭")}</button>
            </div>
          </div>
        </div>
      )}
