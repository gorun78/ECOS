          ...
          {selectedAction.parameters.map(param => {
            ...
            return (
              <div key={param.id} className="space-y-1 text-xs">
                <label className={`text-[10px] ${styles.cardTextMuted} font-semibold flex items-center justify-between`}><span>{param.displayName} ({param.id})</span>{param.isRequired && <span className="text-red-500 font-bold">* 必填</span>}</label>
                {isLocked ? (<input type="text" disabled value={actionParams[param.id] || ''} className={`w-full h-8 text-[11px] ${styles.inputBg} border ${styles.cardBorder} rounded px-2.5 ${styles.cardTextMuted} font-mono`} />)
                : param.id === 'new_status_param' ? (<select value={actionParams[param.id] || ''} onChange={e => setActionParams({ ...actionParams, [param.id]: e.target.value })} className={`w-full h-8 text-[11px] ${styles.inputBg} ${styles.inputText} border ${styles.cardBorder} rounded px-2 focus:border-blue-500`}><option value="">-- 请选择目标状态 --</option>{activeObjectType?.id === 'flight' && (<><option value="ON_TIME">ON_TIME (准点)</option><option value="DELAYED">DELAYED (延误)</option><option value="BOARDING">BOARDING (登机中)</option><option value="CANCELLED">CANCELLED (取消)</option></>)}{activeObjectType?.id === 'aircraft' && (<><option value="ACTIVE">ACTIVE (活跃运行)</option><option value="MAINTENANCE">MAINTENANCE (适航检修)</option><option value="INSPECTION">INSPECTION (深度安全安检)</option></>)}</select>)
                : param.dataType === 'date' ? (<input type="date" value={actionParams[param.id] || ''} onChange={e => setActionParams({ ...actionParams, [param.id]: e.target.value })} className={`w-full h-8 text-[11px] ${styles.inputBg} ${styles.inputText} border ${styles.cardBorder} rounded px-2.5 focus:border-blue-500`} />)
                : (<input type="text" placeholder={`请输入 ${param.displayName}`} value={actionParams[param.id] || ''} onChange={e => setActionParams({ ...actionParams, [param.id]: e.target.value })} className={`w-full h-8 text-[11px] ${styles.inputBg} ${styles.inputText} border ${styles.cardBorder} rounded px-2.5 focus:border-blue-500`} />)}
