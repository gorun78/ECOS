...
export default function GlobalGraphView() {
  ...
  return (
    <div className={`flex-1 ${styles.appBg} overflow-auto relative`}>
      ...
      <div className="absolute bottom-3 right-3 flex gap-2"><span className={`text-[10px] ${styles.muted} ${styles.badgeBg} px-2 py-1 rounded font-mono`}>{kgData.nodes.length}N</span><span className={`text-[10px] ${styles.muted} ${styles.badgeBg} px-2 py-1 rounded font-mono`}>{kgData.edges.length}E</span></div>
    </div>
  );
}
