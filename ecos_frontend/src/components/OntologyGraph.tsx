...
// Dynamic icon resolver for runtime icon names
...
// Exported types for backward compatibility (used by OntologyDesigner)
...
interface OntologyGraphProps {
  ...
  linkTypes?: LinkType[];
  /** Pre-built nodes — used by OntologyDesigner */
  nodes?: GraphNode[];
  /** Pre-built edges — used by OntologyDesigner */
  edges?: GraphEdge[];
...
export default function OntologyGraph({
  ...
  // Initialize node positions — supports both ceos_new (objectTypes/linkTypes)
  // and OntologyDesigner (nodes/edges) modes
  useEffect(() => {
    // Mode 1: Pre-built nodes/edges from external (OntologyDesigner)
    if (externalNodes && externalNodes.length > 0) {
      setNodes(externalNodes);
    } else if (objectTypes.length > 0) {
      // Mode 2: Build from objectTypes/linkTypes (ceos_new style)
    ...
    // Handle external edges (both modes)
    if (externalEdges && externalEdges.length > 0) {
      setEdges(externalEdges);
    }
  }, [objectTypes, linkTypes, externalNodes, externalEdges]);
  ...
  return (
    <div
      ref={containerRef}
      className={`relative w-full h-[450px] border rounded-lg overflow-hidden select-none cursor-default ${styles.cardBg} ${styles.cardBorder}`}
    ...
    >
      ...
      <div className={`absolute top-3 left-3 flex flex-col gap-1 ${styles.cardBg}/80 backdrop-blur-sm px-3 py-2 rounded border ${styles.cardBorder} shadow-xs z-10`}>
      ...
      <svg className="w-full h-full pointer-events-none absolute inset-0">
        <defs>
          ...
          >
            <path d="M 0 0 L 10 5 L 0 10 z" className={`${styles.muted} fill-current`} />
          </marker>
        </defs>
          ...
          return (
            <g key={edge.id} className="group pointer-events-auto cursor-pointer" onClick={() => onSelectEdge(edge.id)}>
              <line
                ...
                strokeWidth="2"
                markerEnd="url(#arrow)"
                className={`${styles.borderColor} group-hover:stroke-blue-400 transition-colors`}
              />
              <line
                x1={cx1}
                y1={cy1}
