...
# 一次启动，每 60s 轮询 Worker 至 done/blocked，再跟 Verifier+Synth
...
while True:
    ...
    if wst in ("done", "blocked", "gave_up"):
        print(f"[{round_num}] Worker terminal: {wst}", flush=True)
        ...
        if vst in ("done","archived","gave_up") and sst in ("done","archived","gave_up"):
            ...
            sys.exit(0)
        if wst == "blocked" and last_w != "blocked":
            # auto unblock once
            try:
                subprocess.run(["hermes","kanban","--board",board,"unblock",worker_id], capture_output=True, text=True, timeout=30)
                print(f"[{round_num}] Auto-unblocked {worker_id}", flush=True)
            except Exception as ex:
                print(f"[{round_num}] unblock err: {ex}", flush=True)
    if round_num >= 40:
